...
>
> Backed by **239 deterministic golden-image regression tests** (each diffs `0.0000` cross-vendor), three byte-exact
> data goldens (engine-state JSON, material-graph JSON, audio WAV), a **106-test** suite that runs clean under
...
| **Verification** | 239 golden images @ `DIFF 0.0000` · 113-test suite · AddressSanitizer-clean · graphics-validation-clean |
...
| Cross-platform reproducibility | Not guaranteed at the byte level | Byte-identical CPU ↔ Vulkan ↔ Metal, proven by 239 golden images |
...
- **Golden images (239).** Each is one headless `visual_test` flag compared to its committed reference at
...
- **Unit tests (106).** A suite over the pure-C++ core — math, ECS, render graph, every header-only render and
...
- **One-command cross-platform gate.** `scripts/verify.ps1` runs the Windows/Vulkan tests + the data-golden byte
  matches, then drives a macOS machine over SSH to build the Metal target once and compare **all 239** references
...
ctest --preset windows-msvc-debug          # 113 tests
...
machine over SSH to build the Metal target once and compare **all 239** references at threshold `0.0`. Prints a
...
├── scripts/verify.ps1          Cross-platform gate: Windows tests + data goldens + Mac 239-image compare
