...
>
> Backed by **215 deterministic golden-image regression tests** (each diffs `0.0000` cross-vendor), three byte-exact
...
| **Verification** | 215 golden images @ `DIFF 0.0000` · 109-test suite · AddressSanitizer-clean · graphics-validation-clean |
...
| Cross-platform reproducibility | Not guaranteed at the byte level | Byte-identical CPU ↔ Vulkan ↔ Metal, proven by 215 golden images |
...
- **Golden images (215).** Each is one headless `visual_test` flag compared to its committed reference at
...
- **One-command cross-platform gate.** `scripts/verify.ps1` runs the Windows/Vulkan tests + the data-golden byte
  matches, then drives a macOS machine over SSH to build the Metal target once and compare **all 215** references
...
machine over SSH to build the Metal target once and compare **all 215** references at threshold `0.0`. Prints a
...
├── scripts/verify.ps1          Cross-platform gate: Windows tests + data goldens + Mac 215-image compare
