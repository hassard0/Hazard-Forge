...
> **Status:** Active development — an in-progress engine, not a finished product. The RHI-seam thesis is proven on real hardware: the same engine code renders a full PBR/IBL/post-processed scene on a Vulkan RTX GPU (Windows) and on Apple Metal (M4, macOS), verified by **46 deterministic golden-image regression tests** (plus a machine-readable engine-state JSON golden, a byte-exact **material-graph introspection JSON golden**, **and** a byte-exact audio-WAV golden) that must each diff `0.0000`, and by a **46-test** ctest suite that runs clean under AddressSanitizer. Roughly slices A–BV have shipped — PBR materials, image-based lighting, HDR bloom, SSAO, alpha-blended transparency, glTF scene-graph import, skeletal animation + blending **plus a parameter-driven animation state machine (cross-fade FSM)**, rigid-body physics, GPU instancing, compute particles, immediate-mode debug visualization, an interactive runtime (fixed-timestep loop + flyable camera), a full shadow set (cascaded shadow maps, spot-light shadows, omnidirectional point-light cube shadows), clustered/Forward+ lighting, screen-space reflections, **screen-space global illumination (SSGI) — raw, edge-preserving bilateral spatial denoise, and temporal accumulation (the full GI trilogy)**, **screen-space projected decals**, volumetric fog / light shafts, reflection + irradiance probes (local cubemap GI), temporal anti-aliasing (TAA), CPU frustum culling, **GPU-driven culling + indirect draw**, **GPU multi-draw-indirect (MDI) batching** (144 objects in one indirect call, `gl_DrawID`-indexed per-draw data), a **render graph with automatic resource-state barriers** (Vulkan-synchronization-validation-clean), **multithreaded command recording** (byte-identical 1-vs-N workers), a **data-driven material / shader graph** (expanded node set incl. a **tangent-space normal-map node** + a multi-material scene) with **live runtime material authoring** **and machine-readable graph introspection (JSON / Graphviz DOT)**, a **data-driven post-process stack** (ordered, JSON-authorable: color grade / chromatic aberration / film grain), a **baked-font text / HUD overlay renderer**, a **deterministic integer-mixer audio engine** (byte-exact WAV golden), a **deterministic networking stack** — replication snapshots/deltas **plus a seeded lossy transport + client jitter-buffer interpolation**, **distance-based scene / asset streaming** (per-frame budget + hysteresis), **procedural terrain / heightmap generation** (finite-difference normals) **plus streamed, distance-banded-LOD terrain tiles**, a **playable deterministic game sample**, machine-readable engine-state JSON introspection for agents, a live editor (mouse pick + gizmo drag + shader/scene hot-reload) **plus a docked ImGui editor shell (Scene Hierarchy / Inspector / Stats around a central Viewport)**, and a macOS windowed Metal viewport. Past the breadth phase, recent slices mark a **depth pivot** into real-time GI (SSGI raw → denoise → temporal) and networking (state replication → lossy transport + interpolation).
...
4. **Engine modules** — `scene/` (incl. distance-based `streaming`), `render/` (render graph + barrier solver + frustum/GPU-cull/TAA + screen-space decals + SSGI: raw/bilateral-denoise/temporal-accumulate), `terrain/` (procedural heightmap + mesh gen + streamed-tile LOD), `asset/` (glTF + HDR env), `anim/`, `physics/`, `material/` (shader graph + NormalMap node + codegen + runtime compile + live authoring + graph introspection), `ui/` (baked-font text / HUD overlay), `audio/` (integer mixer + WAV writer), `net/` (deterministic snapshot/delta replication + seeded lossy transport + client jitter-buffer interpolation), `game/` (roll_game), `runtime/`, `editor/` (incl. docked ImGui shell), `debug/`. All depend only on `rhi/` + `math/`; the backend-agnostic subset compiles into `hf_core` for the sanitized unit tests.
...
│   ├── render/                 RenderGraph + barrier solver + frustum / gpu_cull / taa + csm / spot / point_shadow / clustered / ssr / ssgi (+ bilateral denoise + temporal accum) / volumetric / probe / decal (header-only math)
...
│   ├── net/                    deterministic snapshot/delta replication (authority/replica) + seeded lossy transport + client jitter-buffer interpolation (transport)
...
│   ├── editor/                 picking, gizmo, introspect (engine-state JSON) + editor_panel_data (pure-CPU panel model) + docked ImGui editor shell
...
├── metal_headless/             Standalone no-Conan/no-SDL Metal target (visual_test, 46 showcases)
...
│   │                           graph_introspect/roll_game/audio/replication/net_transport/introspect/
│   │                           editor_panels/live_editor/text/...) + rhi_smoke
│   ├── golden/metal/           The 46 committed Metal goldens
...
├── scripts/verify.ps1          Cross-platform gate: Windows ctest + JSON + audio goldens + Mac 46-golden compare
...
ctest --preset windows-msvc-debug          # 46 tests
...
./build-metal/visual_test out.png  # default scene (one of 46 showcase flags)
```
...
Runs the Windows/Vulkan ctest (plus the engine-state JSON-golden and audio-WAV-golden byte matches) locally and drives the bench Mac over SSH to build `metal_headless` once and golden-compare **all 46** Metal goldens at threshold `0.0`. Prints a per-golden DIFF table and an overall `VERIFY: PASS/FAIL`. (`-SkipWindows` / `-SkipMac` run one half.)
...
- ~~Networking transport beyond the in-process replication channel~~ — **first step shipped** (a seeded, deterministic lossy transport — latency / loss / reorder — + a client jitter-buffer with interpolation, modeled without real sockets); real sockets (UDP/TCP) + prediction / reconciliation still open.
```
