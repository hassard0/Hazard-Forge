...
│   ├── render/                 RenderGraph + barrier solver + frustum / gpu_cull / taa + csm / spot / point_shadow / clustered / ssr / ssgi (+ bilateral denoise + temporal accum) / volumetric / probe / decal / mdi / bindless (header-only math)
...
├── metal_headless/             Standalone no-Conan/no-SDL Metal target (visual_test, 49 showcases)
...
│   │                           gpu_cull/mdi/bindless/parallel_record/decal/post_stack/ssgi/shader_graph/runtime_material/
...
│   ├── golden/metal/           The 49 committed Metal goldens
...
├── scripts/verify.ps1          Cross-platform gate: Windows ctest + JSON + audio goldens + Mac 49-golden compare
...
ctest --preset windows-msvc-debug          # 49 tests
...
./build-metal/visual_test out.png  # default scene (one of 49 showcase flags)
```
