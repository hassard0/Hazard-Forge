...
**animation retargeting** across differently-proportioned skeletons + **animation layering** (clip notifies / additive
poses / montages, with a notify→gameplay-event bridge) + **morph targets / blend shapes**, a **gameplay ability system**
(integer attributes / effects / cooldowns, lockstep+rollback) with a **hierarchical gameplay-tag layer** and **area
targeting shapes + a gameplay-cue event stream**, **behavior-tree depth + utility AI** (parallel nodes / stateful
decorators / service nodes), **cinematic sequencer tracks** (camera-cut / audio / material / sub-sequence nesting),
**first-class integer splines** (roads / scatter / camera rails),
...
| **Fracture & destruction** (`hf::sim::fract` / `hf::sim::fhull`) | Voronoi cell decomposition → bonded-cluster break → falling rubble, bit-exact and replayable — now with **exact integer convex-cell shard hulls** (`fhull`) that rest face-to-face through the GJK/EPA solver, plus debris and dust |
