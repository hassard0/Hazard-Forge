...
│   ├── render/                 RenderGraph + barrier solver + frustum / gpu_cull / taa + csm / spot / point_shadow / clustered / ssr / ssgi (+ bilateral denoise + temporal accum) / volumetric / probe / decal / mdi / bindless / gpu_driven (MDI+bindless combined) / gpu_culled (compute-cull→MDI+bindless) (header-only math)
...
├── metal_headless/             Standalone no-Conan/no-SDL Metal target (visual_test, 52 showcases)
...
│   ├── golden/metal/           The 52 committed Metal goldens
...
├── scripts/verify.ps1          Cross-platform gate: Windows ctest + JSON + audio goldens + Mac 52-golden compare
...
ctest --preset windows-msvc-debug          # 52 tests
...
./build-metal/visual_test out.png  # default scene (one of 52 showcase flags)
```
...
Runs the Windows/Vulkan ctest (plus the engine-state JSON-golden and audio-WAV-golden byte matches) locally and drives the bench Mac over SSH to build `metal_headless` once and golden-compare **all 52** Metal goldens at threshold `0.0`. Prints a per-golden DIFF table and an overall `VERIFY: PASS/FAIL`. (`-SkipWindows` / `-SkipMac` run one half.)
