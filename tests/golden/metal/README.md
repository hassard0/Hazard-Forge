...
`scene.png` is the reference render of the full Slice-F scene (ground checkerboard plane + a 3x3
grid of 9 lit, textured cubes) produced by the **real** Metal RHI backend running headless on an
Apple M4 (`metal_headless/visual_test`). The render is deterministic: a fixed camera, fixed light,
...
./build-metal/visual_test /tmp/metal_scene.png
```
...
pipeline from `shaders/lit.metal` (runtime-compiled MSL), and renders one frame through the
`IRHIDevice` / `ICommandBuffer` seam — the same calls a sample makes. It reads the frame back via
`CaptureNextFrame()` + `GetCapturedPixels()` and writes a PNG.
...
~/mac-remote-rig/compare.sh tests/golden/metal/scene.png /tmp/out.png 0.0
