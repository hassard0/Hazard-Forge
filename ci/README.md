...
| `macos-metal`    | `[self-hosted, macos, metal]`| headless Metal build -> render + golden-compare **all 189 goldens** (DIFF 0.0) |
...
The job builds `metal_headless` **once**, then for each of the **189** committed goldens runs
...
`scripts/verify.ps1`'s per-golden table is the authoritative, always-current index of all 189
...
and run the **same 189-golden loop**, each compared at threshold `0.0` (every one must report
