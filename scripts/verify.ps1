    ...
    Runs the complete verification gate and exits non-zero on ANY failure:
      ...
      2. Mac / Metal (headless, over SSH on the LAN)
         - tar the repo (excluding build dirs + .git + stray PNGs, KEEPING the tracked goldens),
           ...
           EACH of the 18 committed Metal goldens run visual_test with its showcase flag and compare
           the output to the matching golden with threshold 0.0 (every pair must be DIFF 0.0000).
           A per-golden table is printed; the Mac portion passes only if ALL 18 diff 0.0000.
...
# The 18 committed Metal goldens, each produced by a distinct visual_test invocation. Name = the
...
function Invoke-MacVerify {
    ...
    # 3) extract + build ONCE + loop ALL 18 goldens. To avoid the login shell being zsh and to dodge
...
'@
    ...
    Write-Host "Mac verification PASSED (all 18 goldens DIFF 0.0000)" -ForegroundColor Green
}
...
Show 'Mac / Metal (18 goldens)'  $macResult
