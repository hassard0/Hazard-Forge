// Alpha-mask (cutout) full-PBR fragment shader — Slice SC1b. A VERBATIM copy of lit_pbr.frag.hlsl
// plus the glTF alphaMode MASK contract: sample the base-color texture ONCE up front and
// `discard` when its alpha is below the per-draw cutoff (location-6 varying from
// lit_cutout.vert.hlsl, i.e. push-constant material.w; 0 == never discards). Everything after the
// alpha test — lighting, shadows, IBL, emissive — is byte-for-byte lit_pbr.frag. Kept as a SEPARATE
// shader so the golden-locked opaque lit-PBR pipeline is undisturbed; only glTF MASK materials
// (and BLEND, approximated as MASK@0.5 — documented v1) select this pipeline.
...
[[vk::binding(0, 1)]] Texture2D    gBase        : register(t0);  // base-color (sRGB; A = mask alpha)
...
struct PSInput {
    ...
    [[vk::location(6)]] nointerpolation float cutoff : TEXCOORD2;    // SC1b alpha-test cutoff (0 = off)
};
...
float4 main(PSInput i) : SV_Target {
    // --- SC1b ALPHA TEST: one base-color sample serves both the mask and the albedo. Discard as
    // early as possible so masked-out texels never write depth/color (proper cutout silhouettes).
    // Alpha is filtered LINEARLY (never sRGB), so mip-filtered alpha compares correctly. ---
    float4 baseTex = gBase.Sample(gBaseSmp, i.uv);
    if (baseTex.a < i.cutoff) discard;
    ...
    float3 albedo = SrgbToLinear(baseTex.rgb) * i.color;
    ...
    // --- Emissive (sRGB -> linear), ADDED after lighting (same convention as lit_pbr.frag). ---
