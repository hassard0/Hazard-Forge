// Slice BP2 — Deterministic Integer Broadphase: THE CANDIDATE-PAIR GENERATOR EXCLUSIVE PREFIX-SUM of the
// per-body pair counts (the 2nd of the BP2 pair count->scan->emit; the broad_cell_scan / grain_neighbor_scan
// twin). A SINGLE-THREAD allocator ([numthreads(1,1,1)], one thread walks body 0..bodyCount-1 ASCENDING,
// maintaining a running sum) — the DIRECT mirror of broad.h::BuildBroadphasePairs' exclusive scan. The
// exclusive scan is INHERENTLY sequential (perBodyOffset[i] depends on the sum of all PRIOR counts), so a
...
// [perBodyOffset[i], perBodyOffset[i+1]). Pure integer -> bit-identical GPU==CPU + cross-backend; default
// MSL gen suffices (no atomics, no int64).
//
// Buffers (storage, bound at compute bindings 0..2; on Metal these land at buffer(0..2)):
//   b0 perBodyCount  : one uint per body (the BP2 per-body pair count), READ.
