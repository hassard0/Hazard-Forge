// Slice PS2 — Deterministic Persistent Contacts THE PERSISTENT MANIFOLD CACHE compute pass (the 2nd slice of
// FLAGSHIP #21: DETERMINISTIC WARM-STARTED CONTACT CACHING + SLEEPING ISLANDS, hf::sim::persist). ONE THREAD
// PER BOX PAIR (per-pair INDEPENDENT — each thread reads its pair's two bodies+boxes, RECOMPUTES the keyed
// friction manifold (BoxSatStable -> BuildManifold -> the FC1 tangent basis + the PS1 ContactKey per point),
// then MATCHES the persistent cache (a small SSBO scanned in FIXED order) so a matched point INHERITS its prior
// normal+tangent impulses + writes its OWN KeyedManifoldGpu slot; race-free, NO atomics). The SAT + manifold +
// basis logic is copied VERBATIM from engine/sim/convex.h::BoxSat/BoxSatStable + BuildManifold +
// ClosestPointsOnSegments and engine/sim/fric.h::BuildFrictionPoints/MakeTangentBasis (the SAME ops as
// fric_points.comp); the key build copies engine/sim/persist.h::MakeContactKey VERBATIM and the cache match
// copies persist.h::MatchCache VERBATIM (the SAME fixed scan/match order) so the GPU inherited-impulse manifold
// (+ its keys) is byte-identical to the CPU persist::BuildKeyedManifold + MatchCache -> the host GPU==CPU memcmp
// catches any divergence.
//
// INTEGER WIDTH (the determinism crux, the FC2/CX1 lesson): fxmul/fxdiv/FxISqrt + the FxDot/FxCross Q16.16
// products use int64_t. DXC -spirv compiles int64 (the Int64 capability, the fric_points.comp pattern); glslc
// (the Metal HLSL->SPIR-V->MSL frontend) CANNOT parse int64_t in HLSL, so this shader is VULKAN-SPIR-V-ONLY (in
// the Vulkan compile list, NOT in the Metal hf_gen_msl list — UNLIKE PS1's persist_key.comp which IS MSL-native
// because the KEY alone is pure int32; the MANIFOLD this caches is int64). The Metal --persist-cache showcase
// runs the CPU persist::BuildKeyedManifold + MatchCache over the same pairs -> byte-identical to this GPU result
// BY CONSTRUCTION, while the Vulkan side carries the GPU==CPU bit-identity proof.
//
// cacheEnabled=0 -> write a cleared KeyedManifoldGpu (count=0) for every pair (the disabled-path no-op).
//
// Buffers (storage, bound at compute bindings 0..3):
//   b0 gPairs     : the box-pair array (FxBody A + FxBox A + FxBody B + FxBox B per pair) + the global body
//                   indices (bodyAIdx, bodyBIdx) for the key, READ.
//   b1 gCache     : the persistent cache — CachedContactGpu {ContactKey key; int normalImpulse, ti1, ti2}, READ.
//   b2 gManifolds : the KeyedManifoldGpu array (count + 4 FricPoint + 4 ContactKey) per pair, WRITE.
...
// std430 KeyedPair mirror: SatPair (FxBody A (16) + FxBox A (3) + FxBody B (16) + FxBox B (3) = 38) + the two
// global body indices (bodyAIdx, bodyBIdx) for the key = 40 x int32 (160 bytes).
struct KeyedPair {
    FxBody bodyA;
    int    hAx, hAy, hAz;
    FxBody bodyB;
...
// std430 ContactKey mirror (engine/sim/persist.h::ContactKey): 4 x uint32 (16 bytes).
struct ContactKey {
    uint bodyA;
    uint bodyB;
    uint axisIndex;
...
// std430 CachedContact mirror (engine/sim/persist.h::CachedContact): ContactKey (4xu32) + 3 x int32 impulses
// = 7 x int32 (28 bytes).
struct CachedContactGpu {
    ContactKey key;
    int normalImpulse;
    int tangentImpulse1;
    int tangentImpulse2;
...
// std430 FricPoint mirror (engine/sim/fric.h::FrictionPoint, the host packs it into THIS 15 x int32 form for
// the memcmp): point.xyz, normal.xyz, t1.xyz, t2.xyz, normalImpulse, tangentImpulse1, tangentImpulse2.
struct FricPointGpu {
    int px, py, pz;
    int nx, ny, nz;
    int t1x, t1y, t1z;
...
// std430 KeyedManifoldGpu mirror (engine/sim/persist.h::KeyedFrictionManifold packed for the memcmp): count
// (uint32) + 4 FricPointGpu (60 each) + 4 ContactKey (16 each) = 4 + 240 + 64 = 308 bytes (77 x int32).
...
struct PersistCacheParams {
    int4 cfg;   // x=pairCount, y=cacheEnabled, z=cacheCount, w=_
};

[[vk::binding(0, 0)]] RWStructuredBuffer<KeyedPair>          gPairs     : register(u0);
[[vk::binding(1, 0)]] RWStructuredBuffer<CachedContactGpu>   gCache     : register(u1);
[[vk::binding(2, 0)]] RWStructuredBuffer<KeyedManifoldGpu>   gManifolds : register(u2);
[[vk::binding(3, 0)]] RWStructuredBuffer<PersistCacheParams> gParams    : register(u3);

// ===== VERBATIM Q16.16 toolbox (== fric_points.comp / convex.h / fric.h) =====
...
// ===== VERBATIM fric.h::MakeTangentBasis =====
...
// ===== VERBATIM engine/sim/persist.h::MakeContactKey (order-normalize bodyA<=bodyB, axis/feature as-is) =====
...
void main(uint3 gid : SV_DispatchThreadID) {
    ...
    // Cleared KeyedManifoldGpu (count=0, every field zero) — the disabled no-op + the separated default.
    KeyedManifoldGpu mz;
    mz.count = 0u;
    {
        FricPointGpu z;
        ...
        ContactKey kz; kz.bodyA = 0u; kz.bodyB = 0u; kz.axisIndex = 0u; kz.featureIndex = 0u;
    ...
    // minIndex IS sat.axisIndex (the BoxSatStable result the PS1 key needs).
    uint satAxisIndex = minIndex;
    ...
    int3 nAB = mNormal;
    if (FxDot(nAB, t) < 0) nAB = int3(-nAB.x, -nAB.y, -nAB.z);
    int3 tb1, tb2;
    MakeTangentBasis(nAB, tb1, tb2);

    KeyedManifoldGpu km = mz;
    km.count = mCount;
    for (uint i = 0u; i < mCount; ++i) {
        FricPointGpu fp;
        fp.px = mPoints[i].x; fp.py = mPoints[i].y; fp.pz = mPoints[i].z;
        fp.nx = nAB.x; fp.ny = nAB.y; fp.nz = nAB.z;
        fp.t1x = tb1.x; fp.t1y = tb1.y; fp.t1z = tb1.z;
        fp.t2x = tb2.x; fp.t2y = tb2.y; fp.t2z = tb2.z;
        fp.ni = 0; fp.ti1 = 0; fp.ti2 = 0;   // accumulators ZEROED at build (BuildKeyedManifold contract)

        // ===== PS2: the PS1 ContactKey (VERBATIM MakeContactKey), then the cache MATCH (VERBATIM MatchCache) ==
        ContactKey key = MakeContactKey(p.bodyAIdx, p.bodyBIdx, satAxisIndex, i);
        // Scan the cache in FIXED order for the FIRST entry with an equal key -> inherit its impulses.
        for (int ce = 0; ce < cacheCount; ++ce) {
            if (ContactKeysEqual(gCache[ce].key, key)) {
                fp.ni  = gCache[ce].normalImpulse;
                fp.ti1 = gCache[ce].tangentImpulse1;
                fp.ti2 = gCache[ce].tangentImpulse2;
                break;
            }
        }
