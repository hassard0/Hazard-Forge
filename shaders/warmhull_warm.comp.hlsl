// Slice WH3 — Warm-Started Hull Contacts: THE ACCUMULATED WARM-STARTED SOLVER (the core-solve money beat, the
// 3rd slice of FLAGSHIP #26: WARM-STARTED HULL CONTACTS + ROBUST DETERMINISTIC STACKING, hf::sim::warmhull). The
// GPU mirror of engine/sim/warmhull.h::StepWarmHullWorld/N — ONE THREAD ([numthreads(1,1,1)]) runs the WHOLE
// StepWarmHullWorldN over the SMALL hull set: K ticks of predict-integrate -> all-pairs narrowphase ->
// build-keyed-manifold -> MATCH the persistent cache (warm-seed) -> the ACCUMULATED warm normal solve -> rewrite
// the cache -> POSITION de-penetration, copying warmhull.h::StepWarmHullWorld VERBATIM. StepWarmHullWorld IS
// manifold::StepHullWorldHardened (== hull_step_hardened.comp) with EXACTLY step (3) — the impulse solve —
// replaced by the warm-started accumulated solve over the cache; steps (1) integrate, (2) FULL inertia, (4)
// de-pen are UNCHANGED. So a tilted stack HOLDS with global damping OFF where the non-accumulated hardened step
// teeters. The final gBodies + gCache array is byte-identical to the CPU StepWarmHullWorldN -> the host GPU==CPU
// memcmp catches any divergence.
//
// THE PERSISTENT CACHE (the WH3 addition over hull_step_hardened.comp): the accumulated impulses persist ACROSS
// ticks in gCache (a flat CachedHullContactGpu list + a live count in gParams), READ+WRITE, surviving the chunked
// 1-tick/dispatch boundaries exactly as the body buffer does. Each tick: build the keyed manifold per pair (the
// WH1 tagged-clip key derivation ported VERBATIM), MATCH gCache by key (warmhull.h::MatchHullCache), run the
// world Gauss-Seidel warm solve (SolveHullManifoldWarm), then REWRITE gCache to this tick's solved contacts
// (UpdateHullCache — absent keys evicted). The key derivation mirrors warmhull.h::ClipFaceAgainstFaceTagged +
// MakeHullContactKey BYTE-FOR-BYTE.
//
// CHUNKED 1 TICK / DISPATCH (the documented Windows ~2s TDR rule, [[hazard-forge-gpu-tdr-chunking]]): the warm
// step is HEAVIER than the hardened step (the GJK/EPA/clip + the key derivation + the accumulated solve + the
// cache rewrite), so the host dispatches it ONE tick per dispatch over the SAME body + cache buffers (a
// ComputeToComputeBarrier between ticks) — the buffers carry the exact state, so the result is BIT-IDENTICAL to
// one big dispatch; 1 tick is far under the TDR window by construction. ticks here is the per-dispatch tick
// count (== 1).
//
// SINGLE THREAD: the world step is order-dependent (the world Gauss-Seidel + the cache rewrite + the in-place
// de-pen), so the GPU mirror MUST be single-thread (the hull_step_hardened.comp convention). The hull set is small.
//
// INTEGER WIDTH (the MF4/WH2 lesson): the whole chain is int64 (GJK/EPA + the SH clip + the full inertia + the
// accumulated solve). DXC -spirv compiles int64; glslc CANNOT, so this shader is VULKAN-SPIR-V-ONLY (NOT in
// hf_gen_msl). The Metal --wh3-warm runs the CPU StepWarmHullWorldN over the same world -> byte-identical to this
// GPU result BY CONSTRUCTION, while the Vulkan side carries the GPU==CPU memcmp proof.
//
...
//   b2 gCache  : the persistent cache — a flat CachedHullContactGpu list, READ+WRITE (the live count is gParams.cacheCount).
//   b3 gParams : the step config + counts + the live cacheCount, READ+WRITE (cacheCount updated each tick).
...
// A tagged polygon vertex: world-space pos (SAME bits as ClipFaceAgainstFace) + provenance tag + incEdgeOut.
...
// ClipFaceAgainstFaceTagged (== warmhull.h) — mirrors ClipFaceAgainstFace positions BYTE-FOR-BYTE + carries tags.
...
// MakeHullContactKey (== warmhull.h) — order-normalize + fold the role bit. PURE INT32.
...
// A keyed multi-point manifold (== warmhull.h::KeyedHullManifoldWH2, the relevant fields).
struct KeyedManifold { Manifold m; HullContactKey keys[4]; int normalImpulse[4]; };

// BuildKeyedManifold (== warmhull.h::BuildKeyedHullManifold + BuildHullContactKeysForPair) — runs the GJK/EPA
// narrowphase ONCE (NOT twice — the driver rejects the doubled chain) and produces BOTH the multi-point manifold
// (positions/depths/normal — byte-identical to HullContactMulti, the SAME control flow inlined) AND the WH1
// tagged key per point in the SAME deepest-first reduce, so the tags ALIGN index-for-index with the points.
// normalImpulse starts 0 (cold).
KeyedManifold BuildKeyedManifold(uint bodyAIdx, FxHull hA, int4 oA, int3 pA,
    ...
    // --- The narrowphase (== HullContactMulti, inlined so the GJK/EPA runs ONCE and feeds BOTH the manifold +
    //     the key derivation). ---
    GjkOut g = Gjk(hA, oA, pA, hB, oB, pB);
    ...
    if (didFallback) {
        // The single-point witness fallback (== HullContactMulti) — refFaceIdx/tag stay the sentinel.
        km.m.count = 1u; km.m.pts[0] = witnessMid; km.m.depths[0] = e.depth; km.m.normal = nrm;
        refFaceIdx = 0xFFFFu;
    } else {
        // The deepest-first reduce (== HullContactMulti), carrying the tag with each kept point.
        int deepest = 0;
        for (int k = 1; k < candN; ++k) if (candDepth[k] > candDepth[deepest]) deepest = k;
...
void main(uint3 gid : SV_DispatchThreadID) {
    ...
    for (int tk = 0; tk < ticks; ++tk) {
        // (1) predict-integrate dynamic bodies + per-tick damping (== hull_step_hardened).
        ...
        // (2) world inverse inertias, once per tick, FULL tensor (== hull_step_hardened).
        ...
        // (3 — THE SWAP) the WARM-started accumulated solve over the persistent cache, FIXED i<j order — per pair
        // build keyed manifold -> MATCH the cache (warm-seed) -> SolveHullManifoldWarm with solveIters ACCUMULATED
        // sweeps (the iteration lives INSIDE the solver, mutation IN PLACE so later pairs see earlier updates ==
        // warmhull.h::StepWarmHullWorld). The cache is rewritten INLINE into nextCache as each pair solves (no big
        // per-pair array — keeps the SPIR-V small; the OLD cache (matched read-only) and the NEW cache are disjoint
        // because the matched read uses the pre-tick `cc` while nextCache appends at `nc`).
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                ...
                // MatchHullCache (== warmhull.h) — FIXED-order scan over the OLD cache, first match wins (warm-seed).
                ...
                SolveHullManifoldWarm(bi, bj, ia, ib, km, restitution, (uint)solveIters);  // prime + solveIters sweeps
                body[i] = bi; body[j] = bj;
                // UpdateHullCache (== warmhull.h) — append this pair's converged accumulators (absent keys evicted).
                uint c = km.m.count < 4u ? km.m.count : 4u;
                for (uint pp = 0u; pp < c; ++pp) {
                    if (nc >= HF_WH_MAX_CACHE) break;
                    nextCache[nc].key = km.keys[pp];
                    nextCache[nc].normalImpulse = km.normalImpulse[pp];
                    ++nc;
                }
            }
        }
        // Swap nextCache -> cache for next tick (the OLD cache is dead after the pair loop).
        cc = nc;
        for (int co2 = 0; co2 < nc; ++co2) cache[co2] = nextCache[co2];

        // (4) position de-penetration — posIters sweeps, HullContactMulti, m.depths[0]+m.normal (== hardened).
    ...
    gParams[0].c2.w = cc;   // the LIVE cache count (carried to the next dispatch / read back by the host)
}
