// Clustered / Forward+ lit fragment shader (Slice AG). Same PBR + dim directional fill + procedural
// sky IBL machinery as lit.frag / lit_point.frag, PLUS a clustered point-light loop that makes
// HUNDREDS of point lights affordable:
...
// flat fragment buffer slots on Metal). Lights are stored in VIEW space (CPU pre-transforms them),
// so this shader does the clustered point lighting in view space — it already has `view`.
//
...
// formulas here are the verbatim mirror of engine/render/clustered.h.
//
// Clustered FrameData layout (own struct; fits kFrameUboSize=1024 -> 304 bytes). The --clustered-shot
// showcase fills the per-frame UBO with THIS layout. See docs/.../clustered-lighting-design.md.
struct FrameData {
