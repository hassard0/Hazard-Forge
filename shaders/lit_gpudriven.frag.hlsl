// Slice CB — fully-GPU-driven lit fragment shader (MDI + bindless capstone). VULKAN-ONLY. Identical to
// lit_bindless.frag in EVERY lighting/shadow/IBL computation; the base color is sampled from the BINDLESS
// ARRAY by the per-draw index that lit_gpudriven.vert read from PerDraw[gl_DrawID].texIndex and passed
// FLAT (the ONLY difference from the shared lit.frag, exactly as in BZ):
//
//   float3 tex = gTextures[NonUniformResourceIndex(i.texIndex)].Sample(gBindlessSmp, i.uv).rgb * i.color;
//
// `gTextures[]` is the unbounded runtime descriptor array (space 4) of every scene texture, bound ONCE
// via BindBindlessTextures; `i.texIndex` arrives flat from the GPU-driven vertex shader (no push constant
// — the whole multi-material scene draws via ONE DrawIndexedMultiIndirect). NonUniformResourceIndex marks
// the index as potentially non-uniform across a wavefront (DXC lowers it to SPV_EXT_descriptor_indexing).
// The bindless shared sampler (gBindlessSmp) is LINEAR/REPEAT — IDENTICAL to the device default sampler
