// Slice BP3 — Deterministic Integer Broadphase: THE BROADPHASE-DRIVEN BOX WORLD STEP (the scaling beat) compute
// pass (the 3rd slice of FLAGSHIP #23: hf::sim::broad). ONE THREAD ([numthreads(1,1,1)]) runs the WHOLE
// StepConvexWorldBPN over a LARGE body set (256+ dynamic boxes + a static floor) — predict-integrate ->
// per-tick re-broadphase (the candidate pair set, AABB-culled, visited in (i,j)-ASCENDING order) -> world
// Gauss-Seidel impulse over the candidate pairs -> POSITION de-penetration over the candidate pairs, K ticks —
// copying engine/sim/broad.h::StepConvexWorldBP / StepConvexWorldBPN. The final gBodies array is byte-identical
// to the CPU broad::StepConvexWorldBPN reference -> the host GPU==CPU memcmp catches any divergence.
//
// THE BROADPHASE ON THE GPU (the bit-transparency, BP2's proof carried to the full dynamics): the CPU
// StepConvexWorldBP re-builds the candidate pair list each tick (the BP2 grid stencil over dynamics + a
// dynamic-vs-static all-pairs pass) and SORTS it by (i,j) before the solve/de-pen sweeps. BP2 proved that
// sorted candidate set is BYTE-IDENTICAL to the all-pairs i<j set restricted to the AABB-overlapping pairs.
// So this shader reproduces it EXACTLY by the nested for-i: for-j>i loop GATED on AabbOverlap: that visits the
// AABB-overlapping i<j pairs in (i,j)-ascending order — the SAME set, the SAME order as the CPU's sorted list
// (the static-static skip kept verbatim). The candidate AABBs are snapshotted ONCE per tick from the
// post-integrate positions (matching the CPU's pass-2.5 re-broadphase: the list is FIXED for the tick across
// all solve + de-pen sweeps), so the de-pen position pushes never change which pairs are visited mid-tick.
//
// SINGLE THREAD ([numthreads(1,1,1)]): the world step is order-dependent (the world Gauss-Seidel + the in-place
// de-pen — later pairs see earlier updates), so the GPU mirror MUST be single-thread (the convex_step.comp /
// fpx_solve.comp convention). The body set is large, so the per-tick whole-world step is HEAVY -> the host
// CHUNKS the K ticks into bounded tick-batches with a ComputeToComputeBarrier between (the GJ4 hull_step TDR
// fix shape): the body buffer carries the exact Gauss-Seidel state between chunks, so the chunked result is
// BIT-IDENTICAL to one big dispatch (only the dispatch boundary moves, never a computation).
//
// INTEGER WIDTH (the determinism crux): the inertia fxdiv + the FxDot/FxCross/FxMat3MulVec/quaternion Q16.16
// products use int64_t. DXC -spirv compiles int64 (the Int64 capability, the convex_step.comp pattern); glslc
// (the Metal HLSL->SPIR-V->MSL frontend) CANNOT parse int64_t in HLSL, so this shader is VULKAN-SPIR-V-ONLY (in
// the Vulkan compile list, NOT in the Metal hf_gen_msl list). The Metal --broad-convex runs the CPU
// broad::StepConvexWorldBPN over the same world -> byte-identical to this GPU result BY CONSTRUCTION, while the
// Vulkan side carries the GPU==CPU bit-identity proof. (The AabbOverlap broadphase gate is pure int32 compares;
// it is the int64 narrowphase/solve that pins this Vulkan-only.)
//
// stepEnabled=0 -> write the input bodies back UNCHANGED (the disabled-path no-op).
//
// Buffers (storage, bound at compute bindings 0..2):
//   b0 gBodies : the body array (N x FxBody), READ+WRITE (stepped in place over the chunk's ticks).
//   b1 gBoxes  : the box half-extents (N x BoxGpu int4 {hx,hy,hz,_pad}), READ.
...
#define HF_MAX_BODIES 260                      // the large-scene cap (a static floor + 256+ dynamic boxes)
...
// The step params (== the host broad::BroadStepConfig.cfg + counts), packed for std430. (cellSize is NOT
// needed in-shader: the AabbOverlap-gated all-pairs loop reproduces the sorted candidate set independent of the
// grid cell size; the CPU's grid is just the faster way to enumerate the SAME pairs — the BP2 equivalence.)
struct StepParams {
...
// ===== VERBATIM Q16.16 toolbox (== convex_step.comp / convex.h / fpx.h) =====
...
// SolveManifoldImpulse over a pair (== convex.h::SolveManifoldImpulse): one inner sweep (iters=1).
void SolveManifoldImpulse(inout FxBody bA, inout FxBody bB, int invIaW[9], int invIbW[9],
                          Manifold m, int restitution) {
...
// ===== The broadphase AABB predicate (== fpx.h::BodyAabb / AabbOverlap — pure int32 compares) =====
// The candidate gate: the BP2 broadphase emits exactly the i<j pairs whose body AABBs overlap (the floor's
// pairs handled by the dynamic-vs-static all-pairs pass). Reproduced here by gating the nested i<j loop on this
// predicate -> the SAME pair SET as the CPU's grid+sort, visited in the SAME (i,j)-ascending order.
bool AabbOverlapBodies(FxBody a, FxBody b) {
    // BodyAabb: {pos - radius, pos + radius} per axis. AabbOverlap: six inclusive compares.
...
void main(uint3 gid : SV_DispatchThreadID) {
    ...
    // Load the bodies + boxes into local arrays (the large-scene cap).
    ...
    // ===== StepConvexWorldBPN: `ticks` x StepConvexWorldBP (== broad.h, VERBATIM the convex_step shell with
    // the all-pairs solve/de-pen loops GATED on the AabbOverlap broadphase predicate) =====
    for (int tk = 0; tk < ticks; ++tk) {
        ...
        // (2.5) RE-BROADPHASE the pair gate: snapshot each body's broadphase AABB from the post-integrate
        // positions ONCE per tick (the CPU broad::StepConvexWorldBP builds the candidate list at pass 2.5 and
        // keeps it FIXED across all solve + de-pen sweeps). The de-pen pushes that move bodies later this tick
        // do NOT change which pairs are visited -> bit-exact with the CPU's fixed sorted candidate list.
        int aabbLo[HF_MAX_BODIES * 3];
        int aabbHi[HF_MAX_BODIES * 3];
        for (int ai = 0; ai < n; ++ai) {
            aabbLo[ai * 3 + 0] = body[ai].px - body[ai].radius;
            aabbLo[ai * 3 + 1] = body[ai].py - body[ai].radius;
            aabbLo[ai * 3 + 2] = body[ai].pz - body[ai].radius;
            aabbHi[ai * 3 + 0] = body[ai].px + body[ai].radius;
            aabbHi[ai * 3 + 1] = body[ai].py + body[ai].radius;
            aabbHi[ai * 3 + 2] = body[ai].pz + body[ai].radius;
        }

        // (3) impulse solve — world Gauss-Seidel over the candidate pairs, fixed (i,j)-ascending order. The
        // AabbOverlap gate (snapshot AABBs) makes this the SORTED broadphase candidate list (BP2 equivalence).
        // World inverse inertias recomputed per-pair on the fly (avoids a giant invIW[N][9] local array at the
        // large body count) — byte-identical to recomputing them once/tick (a pure function of the body's
        // post-integrate orient + box, unchanged during the velocity solve).
        for (int sweep = 0; sweep < solveIters; ++sweep) {
            for (int i = 0; i < n; ++i) {
                for (int j = i + 1; j < n; ++j) {
                    if (body[i].invMass == 0 && body[j].invMass == 0) continue;   // static-static
                    // broadphase gate (snapshot AABBs).
                    bool ov = aabbLo[i*3+0] <= aabbHi[j*3+0] && aabbLo[j*3+0] <= aabbHi[i*3+0] &&
                              aabbLo[i*3+1] <= aabbHi[j*3+1] && aabbLo[j*3+1] <= aabbHi[i*3+1] &&
                              aabbLo[i*3+2] <= aabbHi[j*3+2] && aabbLo[j*3+2] <= aabbHi[i*3+2];
                    if (!ov) continue;
                    ...
                    int4 qi = int4(body[i].ox, body[i].oy, body[i].oz, body[i].ow);
                    int3 iax0 = FxRotate(qi, int3(HF_FPX_ONE, 0, 0));
                    int3 iax1 = FxRotate(qi, int3(0, HF_FPX_ONE, 0));
                    int3 iax2 = FxRotate(qi, int3(0, 0, HF_FPX_ONE));
                    int3 iinv = BoxInvInertiaBody(half[i], body[i].invMass);
                    int iaW[9]; WorldInvInertia(iax0, iax1, iax2, iinv, iaW);
                    int4 qj = int4(body[j].ox, body[j].oy, body[j].oz, body[j].ow);
                    int3 jax0 = FxRotate(qj, int3(HF_FPX_ONE, 0, 0));
                    int3 jax1 = FxRotate(qj, int3(0, HF_FPX_ONE, 0));
                    int3 jax2 = FxRotate(qj, int3(0, 0, HF_FPX_ONE));
                    int3 jinv = BoxInvInertiaBody(half[j], body[j].invMass);
                    int ibW[9]; WorldInvInertia(jax0, jax1, jax2, jinv, ibW);
        ...
        // (4) position de-penetration — posIters sweeps over the candidate pairs, fixed (i,j) order. SAME
        // snapshot-AABB gate (the pair list is fixed for the tick).
