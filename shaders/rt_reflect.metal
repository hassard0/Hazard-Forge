// Slice METAL-RT S3 — Hardware Ray Tracing: METAL HW MIRROR REFLECTIONS (the rt_reflect.comp.hlsl Metal twin).
// The Apple-Metal native MSL kernel proving Metal-HW == CPU rtrace::RenderSceneReflected byte-identical.
// ONE thread per pixel. The PRIMARY closest-hit + the PRIMARY shadow any-hit + the integer shadowed shade is
// the rt_shadow.metal (RT3/S2) body VERBATIM — its output is the "shadowed primary color" cS. S3 then ADDS the
// RT4 one-bounce mirror reflection: (1) generate the integer primary ray (the FROZEN rtrace.h::PrimaryRay fx
// math), run the primary closest-hit drain over the primitive acceleration structure (margin-inflated bounding
// boxes) folding into the running closest by the total order (t, primIndex); (2) cast the primary SHADOW ray
// (an any-hit OR) for the shadowed shade cS; (3) if the primary surface is REFLECTIVE (rtrace.h::ReflectivityFor
// > 0): build the mirror reflection ray {hit.pos + normal*kRtReflEps, FxReflect(primaryDir, normal)} in integer
// fx, run the primary closest-hit drain AGAIN for the reflected ray, shade the reflected hit with its OWN shadow
// ray (the RT3 shadowed shade cR; a reflection MISS -> background), and BLEND cS/cR on the UNPACKED RGBA8
// channels by the integer reflectivity. Bit-identical to the CPU rtrace::RenderSceneReflected, proven memcmp
// HW==CPU. ONE bounce (the reflected hit's own reflectivity ignored).
//
// THE DETERMINISM CONTRACT (do NOT violate; inherited from rt_query.metal/rt_shadow.metal/RT2/RT3): the HW BVH
// is ONLY a candidate-AABB GENERATOR. We NEVER read the query's float distance/attributes; correctness is OWNED
// by OUR fx math + the (t,primIndex) min (primary AND reflection) / the boolean OR (shadow). The driver's float
// AABB-overlap (and the float traversal ray) only WIDEN the candidate set. Every bounding box is INFLATED by
// kRtAabbMargin host-side so the float overlap is a strict SUPERSET of every true fx hit. The reflection ray is
// the SAME deterministic (t,primIndex) min over the SAME accel structure; the reflected shadow + the blend are
// pure integer -> the whole reflected image is bit-exact cross-backend, regardless of the vendor's BVH order.
// int64 via `long` (native in MSL).
//
// This is the THIRD hand-authored .metal in the repo (the rt_query.metal / rt_shadow.metal twin; all others are
...
// IDENTICAL to rt_shadow.metal's binding (so the showcase uploads the SAME HwPrim/HwParams buffers).
...
// RT4 reflection constants (MUST match rtrace.h::kRtReflEps / kRtMirrorSpherePrim and rt_reflect.comp.hlsl:37-38).
...
// global (t,primIndex) tie-break / shade key. IDENTICAL to rt_shadow.metal::GpuPrim / the host HwPrim.
struct GpuPrim {
    int  kind;
...
// IDENTICAL to rt_shadow.metal::GpuParams / the host HwParams. light[] = the directional light dir (unit,
...
// ---- VERBATIM rtrace.h fx math (int64 via long) — copied from rt_shadow.metal:69-91. ----
...
// VERBATIM rtrace.h::IntersectSphere (copied from rt_shadow.metal:96-120).
...
// VERBATIM rtrace.h::IntersectAabb (copied from rt_shadow.metal:122-159).
...
// VERBATIM rtrace.h::AlbedoFor (copied from rt_shadow.metal:161-170).
...
// VERBATIM rtrace.h::ShadeHitShadowed (copied from rt_shadow.metal:179-191) — the integer Lambert with the
// DIFFUSE term GATED by occlusion. occluded -> the pixel keeps only the ambient floor.
...
// VERBATIM rtrace.h::FxReflect (translated from rt_reflect.comp.hlsl:188-192) — r = d - 2*(d·n)*n (n unit;
...
// VERBATIM rtrace.h::ReflectivityFor (translated from rt_reflect.comp.hlsl:195-199) — the fixed per-prim
// integer reflectivity in [0,kOne]. The ground (primIndex 0) is 0.55; the designated mirror sphere is 0.75;
// every other primitive is 0 (matte -> byte-identical to the RT3 shadowed shade).
...
// rtrace.h::TraceClosest tie-break (copied from rt_shadow.metal:196-200).
...
// A closest-hit drain over the accel structure — the primary-ray body of rt_shadow.metal factored into a
// helper (shared by the primary AND the reflection ray; the HW BVH is ONLY a candidate generator, correctness
// in fx). The MSL twin of rt_reflect.comp.hlsl::TraceClosestHw:215-245. Each call declares its OWN
// intersection_query<> instance (function-local), so the primary and the reflected-primary do not share state.
...
// An any-hit OCCLUSION drain over the accel structure — the shadow-ray body of rt_shadow.metal factored into a
// helper (shared by the primary shadow AND the reflected shadow). occluded iff ANY candidate's fx hit has
// t > kRtShadowMinT (an order-independent boolean OR — MAY early-out). The MSL twin of
// rt_reflect.comp.hlsl::TraceAnyHitHw:250-280.
    ...
    uint2 gid [[thread_position_in_grid]])
{
    ...
    uint background = p.counts[2];

    // VERBATIM rtrace.h::PrimaryRay (integer NDC -> image-plane offset -> dir) — rt_shadow.metal:223-235.
    ...
    if (best.primIndex != HF_RT_MISS) {
        ...
        occluded = TraceAnyHitHw(sro, lightDir, gPrims, accel);
    }
    ...
    // Translated VERBATIM from rt_reflect.comp.hlsl:327-362 (matches rtrace::RenderSceneReflected:541-577).
    uint outColor = cS;
    if (best.primIndex != HF_RT_MISS) {
        int refl = ReflectivityFor(best.primIndex);
        if (refl > 0) {
            // The reflection ray: origin offset along the normal (anti-acne), dir = the mirror reflect of
            // the primary ray dir about the surface normal.
            ...
            // The reflected color cR: background on a miss, else the RT3 shadowed shade (its OWN shadow ray).
            uint cR;
