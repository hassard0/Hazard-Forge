// Slice HF3 — Hull Friction + Joints: THE FRICTION-LOCKED HULL WORLD STEP (gripped-on-ramp), the 3rd slice of
// FLAGSHIP #30: HULL FRICTION + HULL JOINTS, hf::sim::hullfric. The GPU mirror of
// engine/sim/hullfric.h::StepWarmFrictionHullWorld/N — ONE THREAD ([numthreads(1,1,1)]) runs the WHOLE
// StepWarmFrictionHullWorldN over the SMALL scene: K ticks of the warmhull 5-pass tick — (1) predict-integrate
// every dynamic body + per-tick damping -> (2) FULL world inverse inertia per body -> (3) all-pairs i<j broadphase
...
// the HF2 warm cone solver (the normal-only solve SWAPPED for SolveHullFrictionWarm), rewrite the cache to this
// tick's contacts -> (4) position de-penetration along the A->B normal -> (5) orientation (already integrated in
// step 1). It IS warmhull_sleep.comp's narrowphase with the sleeping-island logic REMOVED and the normal-only solve
// replaced by the cone solver + the friction cache. The final gBodies + gCache array is byte-identical to the CPU
// StepWarmFrictionHullWorldN -> the host GPU==CPU memcmp catches any divergence.
//
// THE MONEY BEAT: a hull released on a TILTED static hull GRIPS and stays at rest with mu>0 (the friction cone holds
// the gravity-along-ramp load), but SLIDES off at mu=0 — friction is the thing holding it, on general convex hulls,
// deterministic + warm-started.
//
// CHUNKED 1 TICK / DISPATCH (the documented Windows ~2s TDR rule, [[hazard-forge-gpu-tdr-chunking]]): the whole
// friction step (per-tick GJK/EPA + the cone solve + the de-pen narrowphase) is HEAVY, so the host dispatches it
// ONE tick per dispatch over the SAME body + cache buffers (a ComputeToComputeBarrier between ticks) — the buffers
// carry the exact state, so the result is BIT-IDENTICAL to one big dispatch; 1 tick is far under the TDR window.
//
// SINGLE THREAD: the world step is order-dependent (the world Gauss-Seidel + the cache rewrite + the in-place
// de-pen), so the GPU mirror MUST be single-thread (the warmhull_sleep.comp / hull_step.comp convention).
//
// INTEGER WIDTH (the HF1/HF2 lesson): the whole chain is int64 (GJK/EPA + the SH clip + the full inertia + the cone
// solve + the FxLength). DXC -spirv compiles int64; glslc CANNOT, so this shader is VULKAN-SPIR-V-ONLY (NOT in
// hf_gen_msl). The Metal --hf3-step runs the CPU StepWarmFrictionHullWorldN over the same world -> byte-identical to
// this GPU result BY CONSTRUCTION, while the Vulkan side carries the GPU==CPU memcmp.
//
...
//                in gParams.c2.w).
//   b3 gParams : the step config + counts + the live cacheCount + mu, READ+WRITE.
...
// The step params (== the host HullFrictionStepConfig + counts), packed for std430.
struct StepParams {
