// Slice PS3 — Deterministic Persistent Contacts THE WARM-STARTED CONE SOLVER compute pass (the 3rd slice of
// FLAGSHIP #21: DETERMINISTIC WARM-STARTED CONTACT CACHING + SLEEPING ISLANDS, hf::sim::persist). ONE THREAD
// runs the WHOLE StepWarmWorldN over the SMALL body set, K ticks — copying engine/sim/persist.h::
// StepWarmWorld / StepWarmWorldN VERBATIM (the SAME fixed body order, the SAME fixed i<j pair order, the SAME
// per-pair BuildKeyedManifold -> MatchCache -> SolveFrictionWarm accumulated cone solve, the SAME per-tick
// cache rebuild, the SAME face-preferred BoxSatStable, the SAME A->B basis, the SAME split-by-invMass linear
// de-pen, the SAME int64 ops). The final gBodies array is byte-identical to the CPU reference -> the host
// GPU==CPU memcmp catches any divergence.
//
// THE WARM-START (the PS3 headline): vs fric_step.comp (FC4), the impulse pass is the ACCUMULATED solver
// (SolveFrictionWarm) — each contact's normal/tangent impulses are ACCUMULATORS (the TOTAL impulse), the cone
// clamps the TOTAL, each sweep applies only the DELTA; the accumulators are SEEDED from a per-tick persistent
// CACHE (matched by the PS1 ContactKey) + PRIMED once before the sweeps (re-inject last tick's converged
// impulse). AFTER the tick the cache is REBUILT from the converged accumulators for next tick.
//
// SINGLE THREAD ([numthreads(1,1,1)]): the world step is order-dependent (the cache carries across ticks, the
// in-place de-pen + later pairs see earlier updates), so the GPU mirror MUST be single-thread (the
// fric_step.comp / convex_step.comp convention). The body set is small (a floor + a few slabs).
//
// INTEGER WIDTH (the determinism crux): fxmul/fxdiv/FxISqrt + the Q16.16 products use int64_t. DXC -spirv
// compiles int64; glslc (the Metal HLSL->SPIR-V->MSL frontend) CANNOT parse int64 in HLSL, so this shader is
// VULKAN-SPIR-V-ONLY (in the Vulkan compile list, NOT in the Metal hf_gen_msl list). The Metal --persist-warm
// showcase runs the CPU persist::StepWarmWorldN over the same world -> byte-identical to this GPU result BY
...
// ===== VERBATIM Q16.16 toolbox (== fric_step.comp / convex.h / persist.h) =====
...
// ===== The SAT result + BoxSat / BoxSatStable (== convex.h, VERBATIM the fric_step.comp body) =====
...
// ===== The manifold + BuildManifold (== convex.h::BuildManifold, VERBATIM the fric_step.comp body) =====
...
// Per point: the contact point + the A->B normal + the FC1 tangent basis + the 3 impulse ACCUMULATORS + key.
struct Keyed {
    uint  count;
    int3  pt[4];
    int3  n;             // shared A->B normal
    int3  t1, t2;        // shared FC1 tangent basis
    int   jn[4];         // normal accumulator
    int   jt1[4], jt2[4];// tangent accumulators
    ContactKey keys[4];
};

// BuildKeyedManifold (== persist.h::BuildKeyedManifold): the FC2 manifold + the A->B basis + the PS1 key.
...
// ===== The persistent cache (== persist.h::PersistentCache / CachedContact), a flat array in registers =====
// gCacheCount entries, each {key + jn/jt1/jt2}; MatchCache + the per-tick rebuild copied VERBATIM.
...
void SolveFrictionWarm(inout FxBody bA, inout FxBody bB, int invIaW[9], int invIbW[9],
    ...
    // (1) PRIME ONCE: re-inject the seeded total impulse at every point.
    ...
    // (2) accumulated Gauss-Seidel sweeps (apply only the DELTA).
    for (int it = 0; it < iters; ++it) {
        for (uint pi = 0u; pi < kd.count; ++pi) {
            ...
            // TANGENT t1 then t2 (cone on the accumulated normal, accumulated delta).
...
void main(uint3 gid : SV_DispatchThreadID) {
    if (gid.x != 0u) return;   // single thread runs the WHOLE warm-started world step
    ...
    // The persistent cache (== persist.h::PersistentCache), carried across ticks. Starts empty.
    ...
    // ===== StepWarmWorldN: `ticks` x StepWarmWorld (== persist.h, VERBATIM) =====
    for (int tk = 0; tk < ticks; ++tk) {
        ...
        // The fresh cache being rebuilt this tick (== next in the CPU reference).
        ...
        // (3) impulse solve — per pair BuildKeyedManifold -> MatchCache (seed from last tick) ->
        // SolveFrictionWarm (prime + solveIters accumulated sweeps). FIXED i<j order, in place.
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                ...
                // MatchCache: for each point, the FIRST cache entry with an equal key seeds its accumulators.
                for (uint pi = 0u; pi < kd.count; ++pi) {
                ...
                body[i] = bi; body[j] = bj;
                // UpdateCache-style append: store this pair's converged accumulators into the fresh cache.
        ...
        // (5) swap the rebuilt cache in for next tick (absent keys evicted).
