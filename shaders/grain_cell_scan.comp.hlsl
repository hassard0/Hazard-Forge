// Slice GR2 — Deterministic GPU Granular/Sand: the GRID-HASH NEIGHBOR SEARCH cell-table EXCLUSIVE PREFIX-SUM
// of the per-cell grain counts (the 2nd of the GR2 cell-table count->scan->emit; the FL2 fluid_cell_scan /
// FPX2 fpx_pair_scan / CL2 cloth_edge_scan twin). A SINGLE-THREAD allocator ([numthreads(1,1,1)], one thread
// walks cell 0..cellCount-1 ASCENDING, maintaining a running sum) — the DIRECT mirror of
// grain.h::BuildGrainCellTable's exclusive scan. The exclusive scan is INHERENTLY sequential (cellStart[c]
// depends on the sum of all PRIOR counts), so a deterministic SERIAL scan in one thread is correct + simplest
// + GPU==CPU BIT-EXACT.
//
// For each cell c: cellStart[c] = running; running += cellGrainCount[c]. After the loop the SENTINEL
// cellStart[cellCount] = running (== grainCount, the total) so the emit can read cell c's slice as
...
//   b0 cellGrainCount : one uint per cell (the GR2 per-cell count), READ.
//   b1 cellStart      : cellCount+1 uints (the exclusive prefix-sum + the sentinel total), WRITE.
//   b2 gParams        : the GrainGridParams (cfg.x = cellCount), READ.
...
[[vk::binding(0, 0)]] RWStructuredBuffer<uint>            cellGrainCount : register(u0);
...
void main(uint3 gid : SV_DispatchThreadID) {
    ...
    for (uint c = 0u; c < cellCount; ++c) {
        ...
        running += cellGrainCount[c];
    }
    cellStart[cellCount] = running;         // sentinel: == grainCount (the total)
}
