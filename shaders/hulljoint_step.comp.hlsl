// Slice HF4 — Hull Friction + Joints: HULL JOINTS COMPOSED (the one-deterministic-step headline), the 4th slice of
// FLAGSHIP #30: HULL FRICTION + HULL JOINTS, hf::sim::hulljoint. The GPU mirror of
// engine/sim/hulljoint.h::StepJointedHullWorld/N — ONE THREAD ([numthreads(1,1,1)]) runs the WHOLE
// StepJointedHullWorldN over the SMALL scene: K ticks of the HF3 friction tick with the joint constraint passes
// inserted in the joint::StepArticulatedContacts position — (1) predict-integrate every dynamic body + per-tick
// damping -> (J) jointIters Gauss-Seidel sweeps EACH doing {all SolveBallJoint in FIXED list order, then all
// SolveAngularLimit in FIXED list order} -> (2) FULL world inverse inertia per body -> (3) all-pairs i<j broadphase
// (build the HF1 friction manifold: the keyed hull manifold + the per-pair tangent basis on the EPA normal + the
// basis-axis flip field), warm-seed from the persistent FRICTION cache (key + basisAxis), cfg.solveIters sweeps of
// the HF2 warm cone solver, rewrite the cache to this tick's contacts -> (4) position de-penetration along the A->B
// normal -> (5) orientation (integrated in step 1, adjusted by the angular-limit passes in step J). It IS
// hullfric_step.comp with the joint passes added; the SolveBallJoint / SolveAngularLimit bodies are joint.h copied
// VERBATIM. The final gBodies + gCache array is byte-identical to the CPU StepJointedHullWorldN -> the host GPU==CPU
// memcmp catches any divergence. With EMPTY joint/limit lists the joint block is a no-op and this == hullfric_step.
//
// THE MONEY BEAT: a CHAIN of hulls linked by ball joints hangs CONNECTED from a static anchor + a hinged hull DOOR
// holds its angular limit + a hull rests on the ground with friction — all in ONE deterministic tick, bit-identical
// CPU/Vulkan/Metal.
//
// CHUNKED 1 TICK / DISPATCH (the documented Windows ~2s TDR rule, [[hazard-forge-gpu-tdr-chunking]]): the whole
// jointed friction step (the joint sweeps + per-tick GJK/EPA + the cone solve + the de-pen narrowphase) is HEAVY, so
// the host dispatches it ONE tick per dispatch over the SAME body + cache buffers (a ComputeToComputeBarrier between
// ticks) — the buffers carry the exact state, so the result is BIT-IDENTICAL to one big dispatch; 1 tick << the TDR
// window.
//
// SINGLE THREAD: the world step is order-dependent (the joint Gauss-Seidel + the world Gauss-Seidel + the cache
// rewrite + the in-place de-pen), so the GPU mirror MUST be single-thread (the hullfric_step.comp convention).
//
// INTEGER WIDTH (the HF1-HF3 + JT lesson): the whole chain is int64 (GJK/EPA + the SH clip + the full inertia + the
// cone solve + the FxLength + the joint FxRotate/FxQuatMul/FxQuatNormalize). DXC -spirv compiles int64; glslc CANNOT,
// so this shader is VULKAN-SPIR-V-ONLY (NOT in hf_gen_msl). The Metal --hf4-joint runs the CPU StepJointedHullWorldN
// over the same world -> byte-identical to this GPU result BY CONSTRUCTION, while the Vulkan side carries the GPU==CPU
// memcmp.
//
// stepEnabled=0 -> write the input bodies + cache back UNCHANGED (the disabled-path no-op).
//
...
//   b2 gCache  : the persistent FRICTION cache — a flat CachedHullFrictionContactGpu list, READ+WRITE (live count
//                in gParams.c2.w).
//   b3 gParams : the step config + counts + the live cacheCount + mu + the joint counts + jointIters, READ+WRITE.
//   b4 gJoints : the FxJoint ball-joint array (J x FxJoint), READ.
//   b5 gLimits : the FxAngularLimit array (L x FxAngularLimit), READ.
...
#define HF_MAX_JOINTS 16                     // the small-scene ball-joint cap
#define HF_MAX_LIMITS 16                     // the small-scene angular-limit cap
#define HF_JOINT_BALL 0u                     // MUST match joint.h::kJointBall
#define HF_ANG_HINGE 0u                      // MUST match joint.h::kAngularHinge
#define HF_ANG_CONE 1u                       // MUST match joint.h::kAngularCone
...
// std430 CachedHullFrictionContact mirror (engine/sim/hullfric.h::CachedHullFrictionContact): HullContactKey
// (4xu32) + 3 impulses (int32) + basisAxis (int32) = 8 x int32 (32 bytes).
...
// std430 FxJoint mirror (engine/sim/joint.h::FxJoint): bodyA, bodyB (2xu32) + anchorA.xyz + anchorB.xyz (6xi32) +
// kind (u32) + limit (i32) = 10 x int32 (40 bytes), NO padding holes.
struct FxJoint {
    uint bodyA;
    uint bodyB;
    int  aax, aay, aaz;   // anchorA
    int  abx, aby, abz;   // anchorB
    uint kind;
    int  limit;
};

// std430 FxAngularLimit mirror (engine/sim/joint.h::FxAngularLimit): bodyA, bodyB (2xu32) + axis.xyz (3xi32) +
// cosHalfLimit + sinHalfLimit (2xi32) + kind (u32) = 8 x int32 (32 bytes), NO padding holes.
struct FxAngularLimit {
    uint bodyA;
    uint bodyB;
    int  axx, axy, axz;   // axis
    int  cosHalfLimit;
    int  sinHalfLimit;
    uint kind;
};

// The step params (== the host JointedHullStepConfig + counts), packed for std430.
struct StepParams {
    ...
    int4 c3;   // x=gravity.x, y=gravity.y, z=gravity.z, w=mu
    int4 c4;   // x=jointCount, y=limitCount, z=jointIters, w=(reserved 0)
};

[[vk::binding(0, 0)]] RWStructuredBuffer<FxBody>                       gBodies : register(u0);
[[vk::binding(1, 0)]] RWStructuredBuffer<FxHull>                       gHulls  : register(u1);
[[vk::binding(2, 0)]] RWStructuredBuffer<CachedHullFrictionContactGpu> gCache  : register(u2);
[[vk::binding(3, 0)]] RWStructuredBuffer<StepParams>                   gParams : register(u3);
[[vk::binding(4, 0)]] RWStructuredBuffer<FxJoint>                      gJoints : register(u4);
[[vk::binding(5, 0)]] RWStructuredBuffer<FxAngularLimit>               gLimits : register(u5);
...
// ===== HF1 friction layer: the tangent basis (fric.h) + the friction keyed manifold =====
...
// HullFrictionPointGpu (== hullfric.h::HullFrictionPoint): point.xyz + 3 impulse accumulators.
struct HullFrictionPointGpu { int3 pos; int normalImpulse; int tangentImpulse1; int tangentImpulse2; };

// KeyedFrictionManifold (== hullfric.h::HullFrictionManifold): the keyed manifold geometry + the per-pair tangent
// basis on the SIGN-CORRECTED A->B EPA normal + the basisAxis flip field + the per-point accumulators.
struct KeyedFrictionManifold {
    uint count;
    int3 normal;                  // SIGN-CORRECTED A->B (the basis source)
    int3 t1, t2;                  // the per-pair tangent basis
    int  basisAxis;               // fric::LeastAlignedAxis(normal) — the warm-start flip-detect field
    HullContactKey keys[4];
    HullFrictionPointGpu fpts[4];
};

// BuildKeyedFrictionManifold (== hullfric.h::BuildHullFrictionManifold): the FROZEN BuildKeyedManifold (geometry +
// keys) + the FC1 tangent basis on the sign-corrected A->B normal (computed ONCE per pair). Accumulators ZEROED.
KeyedFrictionManifold BuildKeyedFrictionManifold(uint bodyAIdx, FxHull hA, int4 oA, int3 pA,
                                                 uint bodyBIdx, FxHull hB, int4 oB, int3 pB) {
    KeyedFrictionManifold fm;
    fm.count = 0u; fm.normal = int3(0,0,0); fm.t1 = int3(0,0,0); fm.t2 = int3(0,0,0); fm.basisAxis = 0;
    [unroll] for (int z = 0; z < 4; ++z) {
        fm.keys[z].bodyA = 0u; fm.keys[z].bodyB = 0u; fm.keys[z].refFaceId = 0u; fm.keys[z].incVertId = 0u;
        fm.fpts[z].pos = int3(0,0,0); fm.fpts[z].normalImpulse = 0;
        fm.fpts[z].tangentImpulse1 = 0; fm.fpts[z].tangentImpulse2 = 0;
    }

    KeyedManifold km = BuildKeyedManifold(bodyAIdx, hA, oA, pA, bodyBIdx, hB, oB, pB);
    if (km.m.count == 0u) return fm;   // separated -> count 0

    // (3) sign-correct the EPA normal A->B ONCE (== warmhull::SolveHullManifoldWarm step 0 / HF1 build step 3).
    int3 n = km.m.normal;
    int3 ab = int3(pB.x - pA.x, pB.y - pA.y, pB.z - pA.z);
    if (FxDot(n, ab) < 0) n = int3(-n.x, -n.y, -n.z);
    ...
    fm.count = km.m.count;
    fm.normal = n; fm.t1 = t1; fm.t2 = t2; fm.basisAxis = mi;
    // (5) copy the manifold geometry (points) + keys; zero every point's accumulators (the cold-start contract).
    uint cnt = km.m.count < 4u ? km.m.count : 4u;
    for (uint i = 0u; i < cnt; ++i) {
        fm.keys[i] = km.keys[i];
        fm.fpts[i].pos = km.m.pts[i];
        fm.fpts[i].normalImpulse = 0; fm.fpts[i].tangentImpulse1 = 0; fm.fpts[i].tangentImpulse2 = 0;
    }
    return fm;
}

// SolveHullFrictionWarm (== hullfric.h::SolveHullFrictionWarm) — the accumulated, warm-started, cone-clamped cone
// solver. Prime-once with the seeded TOTAL impulse, then `iters` accumulated Gauss-Seidel sweeps: NORMAL then t1
// then t2, the cone on the ACCUMULATED normal. The accumulators ARRIVE SEEDED (MatchHullFrictionCache) or zero.
void SolveHullFrictionWarm(inout FxBody bA, inout FxBody bB, int invIaW[9], int invIbW[9],
                           inout KeyedFrictionManifold m, int mu, int rest, uint iters) {
    if (m.count == 0u) return;
    int invMassA = bA.invMass;
    int invMassB = bB.invMass;
    uint cnt = m.count < 4u ? m.count : 4u;
    int3 n  = m.normal;   // already A->B (sign-corrected at build)
    int3 t1 = m.t1;
    int3 t2 = m.t2;

    // (1) PRIME ONCE: re-inject the seeded TOTAL impulse at every point (the warm-start kick).
    for (uint pi = 0u; pi < cnt; ++pi) {
        int3 p  = m.fpts[pi].pos;
        int3 rA = int3(p.x - bA.px, p.y - bA.py, p.z - bA.pz);
        int3 rB = int3(p.x - bB.px, p.y - bB.py, p.z - bB.pz);
        int3 J = FxAdd(FxScale(n, m.fpts[pi].normalImpulse),
                       FxAdd(FxScale(t1, m.fpts[pi].tangentImpulse1),
                             FxScale(t2, m.fpts[pi].tangentImpulse2)));
        ...
        for (uint pi = 0u; pi < cnt; ++pi) {
            ...
            // ---- NORMAL impulse (accumulated, clamp >= 0) ----
            {
                ...
                if (kn > 0) {
                    ...
                    int newTotal = m.fpts[pi].normalImpulse + djn;
                    if (newTotal < 0) newTotal = 0;
                    int applied = newTotal - m.fpts[pi].normalImpulse;
                    ...
                    int3 dwB = Mat3MulVec(invIbW, FxCross(rB, J));
                    bB.ax += dwB.x; bB.ay += dwB.y; bB.az += dwB.z;
                    m.fpts[pi].normalImpulse = newTotal;
                }
            }

            // ---- TANGENT impulses (t1 then t2), the cone on the ACCUMULATED normal ----
            int coneLo = -fxmul(mu, m.fpts[pi].normalImpulse);
            int coneHi =  fxmul(mu, m.fpts[pi].normalImpulse);
            for (int ti = 0; ti < 2; ++ti) {
                ...
                if (kt <= 0) continue;
                int djt = fxdiv(-vt, kt);
                int prev = (ti == 0) ? m.fpts[pi].tangentImpulse1 : m.fpts[pi].tangentImpulse2;
                ...
                bB.ax += dwB.x; bB.ay += dwB.y; bB.az += dwB.z;
                if (ti == 0) m.fpts[pi].tangentImpulse1 = newTotal;
                else         m.fpts[pi].tangentImpulse2 = newTotal;
            }
        }
    }
}

// ===== HF4 joint passes: SolveBallJoint / SolveAngularLimit (== engine/sim/joint.h, copied VERBATIM) =====
// The joint solvers index the LOCAL body[] array (the CPU's world.bodies). FxRotate / FxNormalize / FxLength /
// FxQuatMul / FxQuatNormalize / FxDot / fxmul / fxdiv are the SAME int64 toolbox above (== fpx.h / joint.h).

// QConj(q): the conjugate {-x,-y,-z,w} (the inverse of a UNIT quaternion). (== joint.h::QConj.)
int4 QConj(int4 q) { return int4(-q.x, -q.y, -q.z, q.w); }

// QNlerp(p, q, t): component-wise normalized lerp p + t*(q - p) (the caller FxQuatNormalize's). (== joint.h::QNlerp.)
int4 QNlerp(int4 p, int4 q, int t) {
    return int4(p.x + fxmul(q.x - p.x, t), p.y + fxmul(q.y - p.y, t),
                p.z + fxmul(q.z - p.z, t), p.w + fxmul(q.w - p.w, t));
}

// WorldAnchor(body, anchorLocal): body.pos + FxRotate(body.orient, anchorLocal). (== joint.h::WorldAnchor.)
int3 WorldAnchor(FxBody b, int3 anchorLocal) {
    int4 q = int4(b.ox, b.oy, b.oz, b.ow);
    return FxAdd(int3(b.px, b.py, b.pz), FxRotate(q, anchorLocal));
}

// SolveBallJoint: project ONE ball joint (== joint.h::SolveBallJoint VERBATIM — the cloth restLen-0 split).
void SolveBallJoint(inout FxBody body[HF_MAX_BODIES], int n, FxJoint j) {
    if (j.bodyA >= (uint)n || j.bodyB >= (uint)n) return;   // out-of-range -> skip
    FxBody a = body[j.bodyA];
    FxBody b = body[j.bodyB];
    int wsum = a.invMass + b.invMass;
    if (wsum == 0) return;                                  // both pinned -> skip
    int3 pa = WorldAnchor(a, int3(j.aax, j.aay, j.aaz));
    int3 pb = WorldAnchor(b, int3(j.abx, j.aby, j.abz));
    int3 d = FxSub(pb, pa);
    int len = FxLength(d);
    if (len == 0) return;                                   // coincident -> no deterministic normal -> skip
    int pen = len;                                          // restLen 0
    int3 nrm = FxNormalize(d);
    int wa = fxdiv(a.invMass, wsum);
    int wb = fxdiv(b.invMass, wsum);
    int3 ca = FxScale(nrm, fxmul(pen, wa));                 // A moves +ca (toward B's anchor)
    int3 cb = FxScale(nrm, fxmul(pen, wb));                 // B moves -cb (toward A's anchor)
    a.px += ca.x; a.py += ca.y; a.pz += ca.z;
    b.px -= cb.x; b.py -= cb.y; b.pz -= cb.z;
    body[j.bodyA] = a;
    body[j.bodyB] = b;
}

// SolveAngularLimit: project ONE angular limit (== joint.h::SolveAngularLimit VERBATIM — swing-twist + cone clamp +
// nlerp inverse-mass apply). Writes only the two bodies' orient.
void SolveAngularLimit(inout FxBody body[HF_MAX_BODIES], int n, FxAngularLimit lim) {
    if (lim.bodyA >= (uint)n || lim.bodyB >= (uint)n) return;   // out-of-range -> skip
    FxBody a = body[lim.bodyA];
    FxBody b = body[lim.bodyB];
    int wsum = a.invMass + b.invMass;
    if (wsum == 0) return;                                      // both pinned -> skip

    int4 qA = int4(a.ox, a.oy, a.oz, a.ow);
    int4 qB = int4(b.ox, b.oy, b.oz, b.ow);
    int4 qrel = FxQuatMul(QConj(qA), qB);                       // B's orientation in A's frame
    int3 axis = int3(lim.axx, lim.axy, lim.axz);

    // --- swing-twist decomposition about `axis` ---
    int3 qrelXyz = int3(qrel.x, qrel.y, qrel.z);
    int proj = FxDot(qrelXyz, axis);
    int4 twist = FxQuatNormalize(int4(fxmul(axis.x, proj), fxmul(axis.y, proj), fxmul(axis.z, proj), qrel.w));
    int4 swing = FxQuatMul(qrel, QConj(twist));                 // the off-axis part (qrel = swing * twist)

    // --- cone clamp the SWING (host cos/sin limit, NO acos) ---
    if (swing.w < lim.cosHalfLimit) {
        int3 sxyz = int3(swing.x, swing.y, swing.z);
        int slen = FxLength(sxyz);
        if (slen != 0) {
            int3 nhat = FxNormalize(sxyz);
            swing = int4(fxmul(lim.sinHalfLimit, nhat.x), fxmul(lim.sinHalfLimit, nhat.y),
                         fxmul(lim.sinHalfLimit, nhat.z), lim.cosHalfLimit);   // re-synth at the limit
        }
    }
    int4 qrelClamped = FxQuatNormalize(FxQuatMul(swing, twist));   // recompose

    // --- the correction targets + the nlerp inverse-mass apply (NO slerp) ---
    int4 qBtarget = FxQuatMul(qA, qrelClamped);
    int4 qAtarget = FxQuatMul(qB, QConj(qrelClamped));
    int wA = fxdiv(a.invMass, wsum);
    int wB = fxdiv(b.invMass, wsum);
    int4 newB = FxQuatNormalize(QNlerp(qB, qBtarget, wB));         // a pinned body (w 0) is NOT rotated
    int4 newA = FxQuatNormalize(QNlerp(qA, qAtarget, wA));
    b.ox = newB.x; b.oy = newB.y; b.oz = newB.z; b.ow = newB.w;
    a.ox = newA.x; a.oy = newA.y; a.oz = newA.z; a.ow = newA.w;
    body[lim.bodyA] = a;
    body[lim.bodyB] = b;
}
...
void main(uint3 gid : SV_DispatchThreadID) {
    ...
    int mu          = gParams[0].c3.w;
    int jointCount  = gParams[0].c4.x;
    int limitCount  = gParams[0].c4.y;
    int jointIters  = gParams[0].c4.z;

    int n = bodyCount;
    if (n > HF_MAX_BODIES) n = HF_MAX_BODIES;
    int nj = jointCount; if (nj > HF_MAX_JOINTS) nj = HF_MAX_JOINTS;
    int nl = limitCount; if (nl > HF_MAX_LIMITS) nl = HF_MAX_LIMITS;

    FxBody body[HF_MAX_BODIES];
    FxHull hull[HF_MAX_BODIES];
    for (int li = 0; li < n; ++li) { body[li] = gBodies[li]; hull[li] = gHulls[li]; }
    FxJoint joints[HF_MAX_JOINTS];
    FxAngularLimit limits[HF_MAX_LIMITS];
    for (int lj = 0; lj < nj; ++lj) joints[lj] = gJoints[lj];
    for (int ll = 0; ll < nl; ++ll) limits[ll] = gLimits[ll];
    ...
    // The live FRICTION cache (loaded from gCache; rewritten each tick; flushed back at the end).
    CachedHullFrictionContactGpu cache[HF_WH_MAX_CACHE];
    ...
    for (int tk = 0; tk < ticks; ++tk) {
        // (1) predict-integrate every dynamic body + per-tick damping (== StepWarmFrictionHullWorld step 1).
        ...
        // (J — THE JOINT PASSES) jointIters Gauss-Seidel sweeps EACH doing {all SolveBallJoint in FIXED list order,
        // then all SolveAngularLimit in FIXED list order} (== joint::StepArticulatedContacts step 2 / hulljoint.h
        // step J VERBATIM). With nj==0 && nl==0 this is a no-op (the no-joints control == hullfric_step).
        for (int jit = 0; jit < jointIters; ++jit) {
            for (int je = 0; je < nj; ++je) SolveBallJoint(body, n, joints[je]);
            for (int le = 0; le < nl; ++le) SolveAngularLimit(body, n, limits[le]);
        }

        // (2) world inverse inertias, once per tick, FULL tensor (every body).
        ...
        // (3 — THE SWAP) the WARM cone solve over the all-pairs i<j contacts, FIXED order, IN PLACE. Build the HF1
        // friction manifold (tangent basis this tick) -> MatchHullFrictionCache (key + basisAxis) -> the cone solve.
        // The converged manifolds are captured to rewrite the cache (step 5a below) AFTER the solve loop.
        CachedHullFrictionContactGpu nextCache[HF_WH_MAX_CACHE];
        int nc = 0;
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                ...
                KeyedFrictionManifold fm = BuildKeyedFrictionManifold((uint)i, hull[i], oi, pi,
                                                                      (uint)j, hull[j], oj, pj);
                if (fm.count == 0u) continue;
                // MatchHullFrictionCache — FIXED-order scan, first entry whose key AND basisAxis match wins.
                for (uint pp = 0u; pp < fm.count && pp < 4u; ++pp) {
                    for (int ce = 0; ce < cc; ++ce) {
                        if (HullContactKeysEqual(cache[ce].key, fm.keys[pp]) && cache[ce].basisAxis == fm.basisAxis) {
                            fm.fpts[pp].normalImpulse   = cache[ce].normalImpulse;
                            fm.fpts[pp].tangentImpulse1 = cache[ce].tangentImpulse1;
                            fm.fpts[pp].tangentImpulse2 = cache[ce].tangentImpulse2;
                            break;
                        }
                    }
                }
                ...
                SolveHullFrictionWarm(bi, bj, ia, ib, fm, mu, restitution, (uint)solveIters);
                body[i] = bi; body[j] = bj;
                // append this pair's converged accumulators (UpdateHullFrictionCache semantics).
                uint c = fm.count < 4u ? fm.count : 4u;
                for (uint pp2 = 0u; pp2 < c; ++pp2) {
                    if (nc >= HF_WH_MAX_CACHE) break;
                    nextCache[nc].key             = fm.keys[pp2];
                    nextCache[nc].normalImpulse   = fm.fpts[pp2].normalImpulse;
                    nextCache[nc].tangentImpulse1 = fm.fpts[pp2].tangentImpulse1;
                    nextCache[nc].tangentImpulse2 = fm.fpts[pp2].tangentImpulse2;
                    nextCache[nc].basisAxis       = fm.basisAxis;
                    ++nc;
                }
            }
        }
        // (5a) Swap nextCache -> cache for next tick (== UpdateHullFrictionCache, absent keys/basis-flips evicted).
        cc = nc;
        for (int co2 = 0; co2 < nc; ++co2) cache[co2] = nextCache[co2];

        // (4) position de-penetration — posIters sweeps, the plain WH3 linear push-apart (== StepWarmFrictionHull-
        // World step 4 VERBATIM): split by inverse mass; a static body (invMass 0) gets a zero share automatically.
