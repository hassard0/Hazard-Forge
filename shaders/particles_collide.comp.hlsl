// Slice PT3 — Deterministic particle-vs-world COLLISIONS: the Q16.16 per-particle INTEGRATE + COLLIDE compute
// pass (the 3rd slice of FLAGSHIP #19: DETERMINISTIC GPU PARTICLES). ONE thread per particle slot (i < count).
// Each thread (1) runs the PT1 integrate (vel += gravity*dt; vel -= vel*dragK; pos += vel*dt; age + death,
// copied VERBATIM from engine/sim/particles.h::IntegrateParticle) then (2) collides its OWN particle with the
// world: a ground PLANE (clamp surface + reflect the downward velocity) then a FIXED-ORDER loop over a static
// SPHERE collider list (project the centre out + reflect the inward velocity), copied VERBATIM from
// engine/sim/particles.h::CollideParticlePlane + CollideParticleSphere. Each particle is INDEPENDENT (it reads
// the SAME read-only collider list, writes only its OWN slot) -> race-free, NO atomics, bit-identical GPU==CPU
// + cross-backend (the GR1/FL1/PT1/PT2 argument extended with a read-only collider input). The EMIT + RECYCLE
// stay HOST-side (single-thread, between dispatches — the deterministic free-list); the GPU dispatch is the
// per-particle integrate+collide.
//
// WHY BIT-IDENTICAL to the CPU reference (the make-or-break): the GPU does ZERO floating point — it consumes
// the host-snapped Q16.16 int FxParticle + ParticleSphereCollider arrays and runs the SAME pure-integer
// fxmul / FxLength(FxISqrt) / FxNormalize(fxdiv) / FxDot the header runs, colliding plane-then-spheres in the
// SAME fixed order. A divergence here vs engine/sim/particles.h is exactly what the host's GPU==CPU memcmp
// catches.
//
// INTEGER WIDTH (the determinism crux, the GR1/FL1/PT1/PT2 lesson): fxmul/fxdiv/FxISqrt use int64_t — IDENTICAL
// to particles.h. The FxLength sum-of-squares, the FxNormalize fxdiv, the FxDot, and the gravity*dt + (1+e)*vn
// fxmul all exceed int32 for Q16.16 world-scale values, so this shader uses int64. HLSL SM6 supports int64_t
// (the grain/fluid/PT2 pattern — DXC -spirv with the Int64 capability). glslc (the Metal HLSL->SPIR-V->MSL
// frontend) CANNOT parse int64_t in HLSL, so this shader is VULKAN-SPIR-V-ONLY (NOT in metal_headless/
// CMakeLists.txt hf_gen_msl); on Metal the --pt3-collide showcase runs the CPU StepEmitIntegrateCollide
...
//                   48 bytes), READ+WRITE.
//   b1 gParams    : { gravity.xyz, dt, dragK, count, sphereCount, groundY, radius, restitution }, READ.
//   b2 gSpheres   : the Q16.16 ParticleSphereCollider array (center.xyz, radius, +pad — std430), READ.
//
// SEAM DISCIPLINE: ABOVE the RHI seam; the only vk mention is the [[vk::binding]] decorations (same as
// particles_forces.comp / grain_integrate.comp), not backend CODE symbols.
...
//   cfg  : x=dragK (Q16.16), y=count, z=sphereCount, w=collideEnabled
...
[[vk::binding(2, 0)]] StructuredBuffer<FxSphere>     gSpheres   : register(t0);
...
void main(uint3 gid : SV_DispatchThreadID) {
    ...
    int sphereCount    = gParams[0].cfg.z;
    int collideEnabled = gParams[0].cfg.w;
    ...
    // (A) IntegrateParticle (VERBATIM particles.h): vel += gravity*dt; drag; pos += vel*dt; age + death.
    p.vx += fxmul(gravx, dt);
    p.vy += fxmul(gravy, dt);
    p.vz += fxmul(gravz, dt);
    ...
    // (B) CollideParticleWorld: only if still ALIVE (a particle that died this step is NOT collided — matches
    // CollideParticleWorld's ALIVE gate after the integrate marks death). collideEnabled=0 -> skip (PT1 path).
    if (collideEnabled != 0 && (p.flags & HF_PT_FLAG_ALIVE) != 0u) {
