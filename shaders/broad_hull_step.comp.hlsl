// Slice BP4 — Deterministic Integer Broadphase: THE BROADPHASE-DRIVEN HULL WORLD STEP (the new-capability beat)
// compute pass (the 4th slice of FLAGSHIP #23: hf::sim::broad). ONE THREAD ([numthreads(1,1,1)]) runs the WHOLE
// StepHullWorldBPN over a MODERATE mixed-hull set (a static floor + ~48-64 dynamic tetra/octa/wedge/box) —
// predict-integrate -> per-tick RE-BROADPHASE (the hull-AABB candidate set, visited in (i,j)-ASCENDING order)
// -> world Gauss-Seidel GJK/EPA impulse over the candidate pairs -> POSITION de-penetration over the candidate
// pairs, K ticks — copying engine/sim/broad.h::StepHullWorldBP / StepHullWorldBPN. This is the GJ4 hull_step
// shell (the gjk::StepHullWorld 5-pass tick, HullContact = Gjk->Epa->ContactManifold narrowphase) with the
// all-pairs solve/de-pen loops GATED on the hull-AABB broadphase predicate — the direct HULL TWIN of BP3's
// broad_convex_step.comp. The final gBodies array is byte-identical to the CPU broad::StepHullWorldBPN reference
// -> the host GPU==CPU memcmp catches any divergence.
//
// THE BROADPHASE ON THE GPU (the bit-transparency): the CPU StepHullWorldBP re-builds the candidate pair list
// each tick (the BP2 grid stencil over dynamics, hull-AABB proxies + a dynamic-vs-static pass) and SORTS it by
// (i,j) before the solve/de-pen sweeps. The candidate set ⊇ every GJK-overlapping i<j pair, so this shader
// reproduces it EXACTLY by the nested for-i: for-j>i loop GATED on the HULL-AABB AabbOverlap: that visits the
// AABB-overlapping i<j pairs in (i,j)-ascending order — the SAME set, the SAME order as the CPU's sorted list
// (the static-static skip kept verbatim). The hull AABBs (BuildHullAabb = the min/max over six Support(+/-X/Y/Z)
// queries) are snapshotted ONCE per tick from the post-integrate positions (matching the CPU's pass-2.5
// re-broadphase: the list is FIXED for the tick across all solve + de-pen sweeps), so the de-pen position pushes
// never change which pairs are visited mid-tick.
//
...
// hull_step.comp / convex_step.comp / fpx_solve.comp convention). The hull GJK/EPA narrowphase is HEAVIER than
// box SAT, so the host CHUNKS at 1 TICK/DISPATCH from the start (the GJ4/BP3 TDR lesson — a single 1-tick
// dispatch is provably ≪ the Windows ~2s TDR watchdog) with a ComputeToComputeBarrier between: the body buffer
// carries the exact Gauss-Seidel state between chunks, so the chunked result is BIT-IDENTICAL to one big
// dispatch (only the dispatch boundary moves, never a computation).
//
// INTEGER WIDTH (the determinism crux, the GJ1-GJ4/CX1-CX4 lesson): fxmul/fxdiv/FxISqrt + the FxDot/FxCross/
// FxRotate/FxNormalize/quaternion Q16.16 products + the GJK sub-distance + the EPA face math + the Support
// queries all use int64_t. DXC -spirv compiles int64 (the Int64 capability, the hull_step.comp pattern); glslc
// (the Metal HLSL->SPIR-V->MSL frontend) CANNOT parse int64_t in HLSL, so this shader is VULKAN-SPIR-V-ONLY (in
// the Vulkan compile list, NOT in the Metal hf_gen_msl list). The Metal --broad-hull runs the CPU
// broad::StepHullWorldBPN over the same world -> byte-identical to this GPU result BY CONSTRUCTION, while the
// Vulkan side carries the GPU==CPU bit-identity proof. (The hull-AABB gate is pure int compares over the int64
// Support coords; it is the int64 narrowphase/solve that pins this Vulkan-only.)
//
...
#define HF_MAX_BODIES 70                     // the moderate-scene cap (a static floor + ~48-64 dynamic hulls)
#define HF_HULL_MARGIN (HF_FPX_ONE / 2)      // the GJK phantom-contact pad — MUST match broad.h::kHullAabbMargin
...
// BuildHullAabb (== broad.h::BuildHullAabb): the hull's tight world AABB = the min/max over the six world
// Support(+/-X/+/-Y/+/-Z) queries. Returns lo in .xyz of outLo and hi in .xyz of outHi.
...
void main(uint3 gid : SV_DispatchThreadID) {
    ...
    for (int tk = 0; tk < ticks; ++tk) {
        ...
        // (2.5) RE-BROADPHASE: snapshot each body's CUBE-PROXY broadphase AABB from the post-integrate position
        // ONCE per tick (the CPU broad::StepHullWorldBP builds the candidate list at pass 2.5 + keeps it FIXED
        // across all solve + de-pen sweeps). The proxy = the hull's true world AABB (BuildHullAabb, six Support
        // queries) inflated to a CUBE centred at the AABB centre with radius = the max half-extent — the SAME
        // proxy the CPU's BP grid uses for the dynamic pairs; for the dynamic-vs-static pairs the CPU uses the
        // RAW hull AABB (a subset of this cube), so the cube-proxy gate here is a SUPERSET of the CPU candidate
        // set, and every superset-only pair is genuinely separated -> a HullContact no-op -> bit-exact with the
        // CPU's fixed sorted candidate list. The de-pen pushes later this tick do NOT change which pairs visit.
        ...
        for (int ai = 0; ai < n; ++ai) {
            ...
            int3 hlo, hhi;
            BuildHullAabb(hull[ai], oa, pa, hlo, hhi);
            int3 cen = int3((hlo.x + hhi.x) / 2, (hlo.y + hhi.y) / 2, (hlo.z + hhi.z) / 2);
            int rx = (hhi.x - hlo.x) / 2, ry = (hhi.y - hlo.y) / 2, rz = (hhi.z - hlo.z) / 2;
            int r = rx; if (ry > r) r = ry; if (rz > r) r = rz;   // the cube proxy half-extent (max axis)
            r += HF_HULL_MARGIN;   // the GJK phantom-contact pad (MUST match broad.h::kHullAabbMargin)
            aabbLo[ai * 3 + 0] = cen.x - r; aabbLo[ai * 3 + 1] = cen.y - r; aabbLo[ai * 3 + 2] = cen.z - r;
            aabbHi[ai * 3 + 0] = cen.x + r; aabbHi[ai * 3 + 1] = cen.y + r; aabbHi[ai * 3 + 2] = cen.z + r;
        }

        // (3) impulse solve — world Gauss-Seidel over the CANDIDATE pairs, fixed (i,j)-ascending order. The
        // cube-proxy AABB gate (snapshot AABBs) makes this the SORTED broadphase candidate list (a superset of
        // the CPU's set, the extras being HullContact no-ops -> byte-identical to the CPU StepHullWorldBP).
        ...
        // snapshot cube-proxy AABB gate (the pair list is fixed for the tick).
