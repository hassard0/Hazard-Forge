// Slice CX4 — Deterministic Convex Rigid-Body Contacts THE FULL CONVEX STEP (a settling stack) compute
// pass (the 4th slice of FLAGSHIP #19: hf::sim::convex). ONE THREAD runs the WHOLE StepConvexWorldN over
// the SMALL body set (all-pairs over the few boxes) — predict-integrate -> all-pairs narrowphase -> impulse
// solve -> POSITION de-penetration, K ticks — copying engine/sim/convex.h::StepConvexWorld /
// StepConvexWorldN VERBATIM (the SAME fixed body order, the SAME fixed i<j pair order, the SAME world-level
// Gauss-Seidel sweeps, the SAME face-preferred BoxSatStable, the SAME split-by-invMass linear de-pen with
// slop+beta, the SAME per-tick damping, the SAME int64 ops). The final gBodies array is byte-identical to
// the CPU reference -> the host GPU==CPU memcmp catches any divergence. The NEW bit vs CX3 (which was
// velocity-only) is the POSITION de-penetration that makes the stack REST not sink.
//
...
// fpx_solve.comp / mc_scan convention). The body set is small (a floor + a few boxes), so this is fine.
//
// INTEGER WIDTH (the determinism crux, the FPX3/CX1/CX2/CX3 lesson): fxmul/fxdiv/FxISqrt + the FxDot/FxCross/
...
// Metal --convex-stack showcase runs the CPU convex::StepConvexWorldN over the same world -> byte-identical
...
#define HF_MAX_BODIES 16                        // the small-scene cap (floor + a few boxes)
...
// ===== VERBATIM Q16.16 toolbox (== convex_solve.comp / convex.h / fpx.h) =====
...
// IntegrateBodyFull on a body local (== fpx.h::IntegrateBodyFull): dynamic translation + orientation.
...
// BoxSat (== convex.h::BoxSat, VERBATIM the convex_solve.comp body). Returns the 15-axis min-pen result.
...
// BoxSatStable (== convex.h::BoxSatStable): BoxSat + the face-preference post-filter (re-run the 6 face axes,
// prefer a face axis within HF_CX_FACE_PREF_EPS of the edge min-pen -> a clean, stable resting normal).
...
// BuildManifold (== convex.h::BuildManifold, VERBATIM the convex_solve.comp body).
...
// SolveManifoldImpulse over a pair (== convex.h::SolveManifoldImpulse): one inner sweep (iters=1), the
// world-level Gauss-Seidel is the outer loop. Mutates bA/bB vel+angVel in place.
...
void main(uint3 gid : SV_DispatchThreadID) {
    ...
    // Load the bodies + boxes into local arrays (the small-scene cap).
    ...
    // ===== StepConvexWorldN: `ticks` x StepConvexWorld (== convex.h, VERBATIM) =====
    for (int tk = 0; tk < ticks; ++tk) {
        ...
        // world inverse inertias, recomputed once per tick from the post-integrate orient.
