// Slice CP1 — Deterministic Rigid<->Fluid Coupling: the BODY->FLUID grid-hash QUERY EXCLUSIVE PREFIX-SUM of
// the per-body gathered counts (the 2nd of the CP1 query count->scan->emit; the GR2 grain_neighbor_scan
// twin). A SINGLE-THREAD allocator ([numthreads(1,1,1)], one thread walks body 0..bodyCount-1 ASCENDING,
// maintaining a running sum) — the DIRECT mirror of couple.h::GatherBodyParticles's exclusive scan. The
// exclusive scan is INHERENTLY sequential (bodyStart[i] depends on the sum of all PRIOR counts), so a
...
//   b0 perBodyCount : one uint per body (the CP1 gathered count), READ.
//   b1 bodyStart    : bodyCount+1 uints (the exclusive prefix-sum + the sentinel total), WRITE.
//   b2 gParams      : the CoupleParams (dim.w = bodyCount), READ.
...
[[vk::binding(2, 0)]] RWStructuredBuffer<CoupleParams> gParams      : register(u2);
