// Slice RT4 — Hardware Ray Tracing: DETERMINISTIC RT MIRROR REFLECTIONS (the csrq stage). ONE thread per
// pixel (gid.x = px, gid.y = py). The PRIMARY ray + the PRIMARY SHADOW ray body is COPIED VERBATIM from
// shaders/rt_shadow.comp.hlsl (RT3): each thread generates its integer primary ray (the frozen
// rtrace.h::PrimaryRay math), opens a RayQuery over the TLAS (gTlas, binding 4) with a FLOAT ray, DRAINS
// EVERY candidate procedural primitive via Proceed() WITHOUT committing or shrinking tMax, runs the FROZEN
// fx Q16.16 IntersectSphere/IntersectAabb on each candidate, folds into the running closest by the total
// order (t, primIndex), then casts the primary SHADOW ray (an any-hit OR) for the shadowed shade cS. THEN,
// if the primary hit is REFLECTIVE (rtrace.h::ReflectivityFor > 0): it casts a REFLECTION ray (the view dir
// mirrored about the fx normal, a SECOND closest-hit drain), shades the reflected hit with its OWN shadow
// ray (the RT3 shadowed shade cR; a reflection MISS -> background), and BLENDS cS/cR on the UNPACKED RGBA8
// channels by the integer reflectivity. Bit-identical to the CPU rtrace::RenderSceneReflected, proven
// memcmp HW==CPU. ONE bounce (the reflected hit's own reflectivity ignored).
//
// THE DETERMINISM CONTRACT (inherited from RT2/RT3): the HW BVH is ONLY a candidate-AABB GENERATOR. We
// NEVER read the query's float t / hit attributes; correctness is OWNED by OUR fx math. The reflection ray
// is the SAME deterministic (t,primIndex) min over the SAME accel structure; the reflected shadow + the
// blend are pure integer -> the whole reflected image is bit-exact cross-backend, regardless of BVH order.
//
...
// in the Metal hf_gen_msl list. On Metal the --rt4-reflect showcase runs the CPU rtrace::RenderSceneReflected
...
// A closest-hit RayQuery drain over the TLAS — VERBATIM the primary-ray body of rt_shadow.comp (shared by
// the primary AND the reflection ray; the HW BVH is ONLY a candidate generator, correctness in fx).
...
// An any-hit OCCLUSION RayQuery — VERBATIM the shadow-ray body of rt_shadow.comp (shared by the primary
