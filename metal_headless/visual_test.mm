...
// --- Color Grading showcase (Slice DB). Mirrors the Vulkan --colorgrade-shot path EXACTLY: the standard
// lit+shadowed scene renders into an HDR RT; the engine's post.frag tonemaps it into an intermediate LDR
// RT (swapchain format); then a fullscreen color_grade.frag pass (reusing post.vert) reads that
// tonemapped color and applies render/color_grade.h's Grade (LiftGammaGain + ASC_CDL + Saturate, mirrored
// in-shader) from a small GradeParams push constant with a CINEMATIC teal-shadows / warm-highlights grade
// -> swapchain. The SAME render/color_grade.h math compiled into color_grade.frag makes the grade
// bit-identical to the Vulkan/CPU path. THE IDENTITY-GRADE NO-OP PROOF (mirrors the Vulkan path,
// backend-portable): the SAME chain at the IDENTITY grade (color_grade.frag's pure pass-through of the
// tonemapped LDR) and the UNGRADED render (post.frag DIRECTLY to the swapchain) are asserted
// BYTE-IDENTICAL — both quantize the SAME post.frag float to the SAME 8-bit swapchain format and the
// identity grade is an exact pass-through, so it holds on every backend. SEPARATE color_grade pipeline +
// the REUSED lit/shadow/sky/post pipelines; existing pipelines/shaders/goldens untouched. ----------------
static int RunColorGradeShowcase(const char* outPath) {
    using math::Mat4; using math::Vec3;
    const uint32_t W = 1280, H = 720;
    auto device = rhi::mtl::CreateMetalDeviceHeadless(W, H);
    const rhi::Format kHdr = rhi::Format::RGBA16_Float;
    ...
    // FIXED CINEMATIC grade (IDENTICAL to the Vulkan --colorgrade-shot path).
    render::grade::GradeParams gp;
    gp.lift   = {0.015f, 0.045f, 0.060f};
    gp.gamma  = {1.05f,  1.02f,  0.98f};
    gp.gain   = {1.10f,  1.00f,  0.90f};
    gp.slope  = {1.02f,  1.00f,  0.98f};
    gp.offset = {0.00f,  0.00f,  0.005f};
    gp.power  = {1.00f,  1.00f,  1.00f};
    gp.saturation = 1.18f;
    ...
    // Scene objects — IDENTICAL to the Vulkan --colorgrade-shot path.
    struct Obj { Vec3 pos; float scale; bool cube; float col[3]; };
    const Obj objs[] = {
    ...
    auto gradeFs = loadMSL("color_grade.frag.gen.metal", "color_grade_fragment");
    auto postFs  = loadMSL("post.frag.gen.metal", "post_fragment");

    rhi::GraphicsPipelineDesc gradeD;
    gradeD.vertex = postVs.get(); gradeD.fragment = gradeFs.get();
    ...
    // UNGRADED reference: scene -> post.frag tonemap -> swapchain directly.
    ...
    // THE IDENTITY-GRADE NO-OP PROOF (backend-portable: the identity grade of the tonemapped LDR and the
    // ungraded render both display the SAME tonemapped LDR -> byte-identical on every backend).
    std::vector<uint8_t> gradedPx, idPx, ungrPx;
    uint32_t gw = 0, gh = 0, iw = 0, ih = 0, uw = 0, uh = 0;
    if (!renderGraded(gp, gradedPx, gw, gh)) return fail("no captured pixels (cinematic grade)");
    if (!renderGraded(render::grade::GradeParams::Identity(), idPx, iw, ih))
        return fail("no captured pixels (identity grade)");
    if (!renderUngraded(ungrPx, uw, uh)) return fail("no captured pixels (ungraded)");

    const bool idEquivalent = (iw == uw) && (ih == uh) && (idPx.size() == ungrPx.size()) &&
                              (std::memcmp(idPx.data(), ungrPx.data(), ungrPx.size()) == 0);
    if (!idEquivalent)
        return fail("identity-grade render != ungraded render — NOT identity-grade equivalent");
    ...
    if (!WritePNG(outPath, gradedPx, gw, gh)) return fail("PNG write failed");
    device->WaitIdle();
...
int main(int argc, char** argv) {
    @autoreleasepool {
        ...
        // --colorgrade <out.png>: analytic color-grading showcase (Slice DB) — the standard lit+shadowed
        // scene tonemapped (post.frag), then a color_grade.frag fullscreen pass applies render/
        // color_grade.h's Grade (lift/gamma/gain + ASC-CDL slope/offset/power + luma-preserving
        // saturation) from a push constant for a cinematic teal-shadows / warm-highlights look. INTERNALLY
        // renders the SAME chain at the IDENTITY grade (color_grade.frag's pass-through) AND the ungraded
        // render (post.frag direct to swapchain) and asserts they are BYTE-IDENTICAL — the identity-grade
        // no-op proof. Mirrors the Vulkan --colorgrade-shot exactly (same scene/camera/grade params; the
        // SAME render/color_grade.h math makes the grade bit-identical).
        ...
        // --reflprobe <out.png>: box-projected cubemap reflections showcase (Slice DA). The SAME box room
        // baked into the reflection cubemap; reflective heroes shaded by reflprobe.frag with the specular
        // reflection direction parallax-corrected to the local probe box. The parallaxStrength=0 render is
        // BYTE-IDENTICAL to the standard infinite-cubemap reflection (lit_probe.frag). Mirrors the Vulkan
        // --reflprobe-shot exactly; new golden refl_probe.png.
        ...
        // colored objects standing on it: the scene is rendered through the camera reflected across the
        // mirror plane into a 2D RT, then the mirror floor samples it. INTERNALLY asserts reflectivity=0
        // == the matte render (BYTE-IDENTICAL). Mirrors the Vulkan --planar-shot; new golden
        // planar_reflection.png.
        if (argc > 1 && std::strcmp(argv[1], "--planar") == 0) {
            const char* out = argc > 2 ? argv[2] : "metal_planar.png";
            try { return RunPlanarShowcase(out); }
            catch (const std::exception& e) { return fail(std::string("exception: ") + e.what()); }
        }
