...
> always-current list (376 showcases). The determinism that underlies all of this — bit-identical Vulkan/Windows ==
...
## Beyond parity — the determinism moat (shipped headlines)

The capabilities where UE5 is not behind but **structurally disqualified**: its float physics core (FPU evaluation order, FMA contraction, vendor math libraries, non-deterministic task scheduling) cannot re-derive a simulation bit-for-bit from inputs, so provable anti-cheat, reproducible what-if replay, and peer-symmetric rollback over a real network are categories it cannot enter. Each entry below composes the already-shipped, golden-gated determinism substrate **read-only / byte-untouched** and carries its slice's honest caveat verbatim. The full write-ups are in ARCHITECTURE "Beyond parity: the determinism-moat headlines"; the runnable proofs live in `benchmarks/` (`docs/BEAT_UE5_PLAN.md` is where the UE5 comparison belongs).

| Capability | Ships? | See it (flag) | Notes (UE5 structural reason + honest caveat) |
|---|---|---|---|
| **Provable anti-cheat** (server-authoritative re-simulation verifier) | ✅ | `--ac1-verify` | `engine/net/authority_verify.h`, golden `ac1_verify`. The server re-simulates a suspect client's *submitted* input stream and compares per-tick outcome digests against the client's *claimed* ones, rejecting a cheater at the **exact divergence tick**. Composes `verdict` (VD1–VD6 re-sim + `DigestSnapshot`) + `net::DesyncDetector` (NS5) read-only; pure-CPU integer. **UE5 can't:** a server cannot re-derive a client's float physics bit-for-bit. *Boundary:* proves **simulation integrity** (outcomes can't be faked), **not** input-level cheats (an aimbot picking legal inputs is a separate statistical layer) |
| **What-if fork replay** (reproducible counterfactual timelines) | ✅ | `--fk1-fork` | `engine/replay/fork.h`, golden `fk1_fork`. Rewind a recorded match to any tick, mutate one input, re-simulate a **bit-exact counterfactual timeline** — the shared prefix is byte-identical, branches diverge causally, each reproducible run-to-run + cross-platform. Composes `replay.h` RP4 `Seek` + `net::CatchUp` (NS6) + verdict VD read-only; pure-CPU integer. **UE5 can't:** float has no reproducible re-derivation and no inverse. *Caveats:* a mutation injects/removes a `verdict` Command (not engine constants / geometry); each forked tick re-clones the world (bit-exact but heavier); FK1 pins a **2-branch** tree (`ForkAt` is general) |
| **Rollback netcode over real UDP** | ✅ | `--nw1-udp` | `engine/net/udp_transport.h`, golden `nw1_udp`. The deterministic rollback netcode runs over **real Winsock/BSD datagrams** — two peers converge to the **identical match digest** over real `127.0.0.1` UDP even with injected reorder/duplicate/loss. Composes `game/duel.h` + `net/session.h` read-only. **UE5 can't:** peer-symmetric rollback needs a bit-exact sim re-derivable from inputs; float Chaos forces authoritative-server + interpolation. *Caveat:* real sockets are the **one documented nondeterministic-I/O exception** (the *outcome* digest is bit-exact); the committed gate is a **same-process two-socket loopback** (OS-assigned ports, ACK+resend, non-flaky) — the true two-process run is in `benchmarks/udp_two_process.md` |
| **The engine's first game** (deterministic rollback-physics knockout duel) | ✅ | `--game1-duel` | `engine/game/duel.h`, golden `game1_duel`. A small-but-complete playable **sumo/knockout duel**: two `fpx` rigid-body avatars, a **shove** ability (GAS1 cost + cooldown), a **State.Stunned** tag (GT1), impact/knockout **cues** (GC1), best-of-3 rounds, ring-out/kill-plane win — composing the whole moat stack (bit-exact deterministic · lockstep+rollback · fork-able what-if · replay-pinnable · anti-cheat-verifiable). Match digest `0x78123003c3a55a37`; composes `verdict`/`ability`/`tags`/`cues`/AC1/FK1 read-only, pure-CPU integer. *Caveat:* a **headless deterministic match simulation + the proofs** — **no live input window yet** (a future slice) |
| **Runnable proofs UE5 loses** (`benchmarks/` — 6 structural proofs) | ✅ | `ctest -R hf_moat_proofs` (or `benchmarks/run_all.ps1` / `run_all.sh`) | `benchmarks/moat_proofs.cpp`, ctest target `hf_moat_proofs` (**not** a golden image — a runnable proof binary that exits 0 iff all six pass; `scripts/verify.ps1` runs it). Six proofs, each with the UE5-disqualification reason: (1) cross-platform determinism, (2) rollback/desync detection, (3) provable anti-cheat, (4) reproducible counterfactual, (5) reproducible procgen, (6) bandwidth (inputs << state). *Honesty carried:* proof 1's full Win/Mac/Linux demo needs three machines (each leg reproducible; Linux/gcc records 135/135); proof 6 is a specific-scene ~442× measurement, not a universal ratio. See `benchmarks/README.md` |
| **Deterministic foliage impostors + cross-fade LOD** (octahedral cell addressing + Bayer-dither LOD) | ✅ | `--fo1-impostor` | `engine/foliage/impostor.h`, golden `fo1_impostor`. The **deterministic core** of octahedral impostors + dithered cross-fade LOD: `OctEncode`/`ImpostorCell` (octahedral atlas-cell addressing) + `CrossFadeWeight` (an integer transition band + ordered Bayer-4×4 screen-door dither that replaces FO4's popping hard swap). All Q16.16/integer, bit-identical CPU/Vulkan/Metal (no trig/float/clock/RNG); composes `foliage.h` read-only. Digest `0x104198cb9fd5dfaf`. *Honest scope:* the **GPU octahedral-atlas bake + textured billboard render is deferred to FO2** — this ships the addressing + cross-fade core only (it does **not** render impostors); the dither is ordered/screen-door, not temporal; the octahedral round-trip is within a pinned LSB band (the cross-compiler identity is the strict-zero guarantee) |
